...
class EnKFMain(BaseCClass):
    ...
    def __init__(self, config, verbose=False, read_only=False):
        """Initializes an instance of EnkfMain.

        Note: @config is a ResConfig instance holding the configuration.
        """
        real_enkf_main = _RealEnKFMain(config, verbose, read_only)
...
class _RealEnKFMain(BaseCClass):
    ...
    def __init__(self, config, verbose=False, read_only=False):
        ...
        c_ptr = self._alloc(res_config, verbose, read_only)
        if c_ptr:
