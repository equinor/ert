...
from ert_shared.storage.extraction import _get_from_server, _post_to_server
import requests
...
def _getExperiment():
    resp: Response = _post_to_server(
        "/gql", json={"query": "{experiments{ensembles{id}}}"}
    )
    answer_json = resp.json()
    # only one experiment ??
    return answer_json["data"]["experiments"][0]
...
def getAllCases():
    ...
    for e in experiment["ensembles"]:
        ensemble = _get_from_server(f"/ensembles/{e['id']}").json()

        name = ensemble["userdata"]["name"]
        cases.append(name)
