...
int main(int argc, char **argv) {
    ...
    install_file(ta, init_file);
    {
        ...
        {
            const enkf_config_node_type *surface_config_node =
                ensemble_config_get_node(ens_config, "SURFACE");
            enkf_node_type *surface_node = enkf_node_alloc(surface_config_node);
            char *init_file1 =
                enkf_config_node_alloc_initfile(surface_config_node, NULL, 0);
            char *init_file2 =
                enkf_config_node_alloc_initfile(surface_config_node, "/tmp", 0);

            test_assert_bool_equal(
                enkf_config_node_use_forward_init(surface_config_node),
                forward_init);
            ...
            test_assert_bool_equal(enkf_node_use_forward_init(surface_node),
                                   forward_init);
            if (forward_init)
                test_assert_bool_not_equal(
                    enkf_node_initialize(surface_node, 0, rng), forward_init);
            // else hard_failure()
            enkf_node_free(surface_node);
            rng_free(rng);
        }
        test_assert_bool_equal(forward_init,
                               ensemble_config_have_forward_init(
                                   enkf_main_get_ensemble_config(enkf_main)));
        ...
        if (forward_init) {
            ...
            enkf_fs_type *fs = enkf_main_get_fs(enkf_main);
            const subst_list_type *subst_list = subst_config_get_subst_list(
                enkf_main_get_subst_config(enkf_main));
            run_arg_type *run_arg = run_arg_alloc_ENSEMBLE_EXPERIMENT(
                "run_id", fs, 0, 0, "simulations/run0", "BASE", subst_list);
            ...
            util_copy_file(init_file, "simulations/run0/Surface.irap");
            {
                ...
                {
                    double value;
                    test_assert_true(enkf_node_user_get(surface_node, fs, "0",
                                                        node_id, &value));
                    test_assert_double_equal(2735.7461, value);

                    test_assert_true(enkf_node_user_get(surface_node, fs, "5",
                                                        node_id, &value));
            ...
            test_assert_true(
                enkf_node_fload(surface_node, "simulations/run0/SURFACE.INC"));
            {
                double value;
                test_assert_true(
                    enkf_node_user_get(surface_node, fs, "0", node_id, &value));
                test_assert_double_equal(2735.7461, value);

                test_assert_true(
                    enkf_node_user_get(surface_node, fs, "5", node_id, &value));
                test_assert_double_equal(2737.0122, value);
            }
            util_clear_directory("simulations", true, true);
        }
