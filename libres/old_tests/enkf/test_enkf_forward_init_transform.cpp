...
bool check_original_exported_data_equal(const enkf_node_type *field_node) {
    ...
    ecl_kw_type *kw_original =
        ecl_kw_fscanf_alloc_grdecl_dynamic(original_stream, "PORO", ECL_DOUBLE);

    enkf_node_ecl_write(field_node, "tmp", NULL, 0);
    FILE *exported_stream = util_fopen("tmp/PORO.grdecl", "r");
    ecl_kw_type *kw_exported =
        ecl_kw_fscanf_alloc_grdecl_dynamic(exported_stream, "PORO", ECL_DOUBLE);
...
int main(int argc, char **argv) {
    ...
    ecl::util::TestArea ta("init_transform");
    ...
    run_arg_type *run_arg = run_arg_alloc_ENSEMBLE_EXPERIMENT(
        "run_id", init_fs, 0, 0, "simulations/run0", "base", subst_list);
