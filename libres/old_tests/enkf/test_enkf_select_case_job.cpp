...
int main(int argc, const char **argv) {
    ...
    ert_test_context_type *test_context =
        ert_test_context_alloc("SELECT_CASE", config_file);

    test_assert_true(ert_test_context_install_workflow_job(
        test_context, "SELECT_CASE", select_case_job));
    {
        enkf_main_type *enkf_main = ert_test_context_get_main(test_context);
        stringlist_type *args = stringlist_alloc_new();
        stringlist_append_copy(args, "OtherCase");

        test_assert_string_not_equal("OtherCase",
        ...
        test_assert_true(ert_test_context_run_worklow_job(test_context,
                                                          "SELECT_CASE", args));
        test_assert_string_equal("OtherCase",
                                 enkf_main_get_current_fs(enkf_main));
