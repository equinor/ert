...
void file_cb(char const *path, char const *name, void *) {
    storage.push_back(std::string(path) + UTIL_PATH_SEP_STRING + name);
}

bool dir_cb(char const *path, char const *name, int, void *) {
    storage.push_back(std::string(path) + UTIL_PATH_SEP_STRING + name);
    return false;
}
...
std::vector<std::string> directory_tree(std::string const &root) {
    storage.clear();
    util_walk_directory(root.c_str(), file_cb, nullptr, dir_cb, nullptr);
...
void test_export_json() {
    ...
    const cJSON *compkey1 =
        cJSON_GetObjectItemCaseSensitive(json, "KEY100:SUBKEY1");
    ...
    auto const strJSON2 = std::string(std::istreambuf_iterator<char>(f2),
                                      std::istreambuf_iterator<char>());
...
void test_export_txt() {
        ...
        util_files_equal("path/parameters__.txt", "path/parameters.txt"));
    {
        ...
        fscanf(stream, "%[^:]:%s %lg %[^:]:%s %lg", key1, subkey1, &v1, key2,
               subkey2, &v2);
        fclose(stream);
