...
int main(int argc, char **argv) {
    ...
    ta.copy_directory_content(root_path);
    {
        ...
        enkf_main = enkf_main_alloc(res_config);
        {
            ...
            test_assert_bool_equal(enkf_node_use_forward_init(gen_kw_node),
                                   forward_init);
            if (forward_init)
                test_assert_bool_not_equal(
                    enkf_node_initialize(gen_kw_node, 0, rng), forward_init);
            // else hard_failure()
        ...
        if (forward_init) {
...

            {
                ...
                {
                    double value;
                    test_assert_true(enkf_node_user_get(
                        gen_kw_node, fs, "MULTFLT", node_id, &value));
