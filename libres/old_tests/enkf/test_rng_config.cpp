...
static void create_config(const char *user_config_file,
...
void test_init() {
    ...
    rng_config_type *rng_config =
        rng_config_alloc_load_user_config(config_file);
    test_assert_string_equal(random_seed,
                             rng_config_get_random_seed(rng_config));
...
static void alloc_reproduced_rng_config(const char *random_seed,
...
void test_reproducibility(const char *random_seed) {
    ...
    alloc_reproduced_rng_config(random_seed, &orig_rng_config, &rep_rng_config,
    ...
    test_assert_int_equal(rng_get_int(orig_rng_0, MAX_INT),
                          rng_get_int(rep_rng_0, MAX_INT));
    test_assert_int_equal(rng_get_int(orig_rng_100, MAX_INT),
                          rng_get_int(rep_rng_100, MAX_INT));
}
...
int main(int argc, char **argv) {
    ...
    test_reproducibility(
        "423543854372895743289507289532423543854372895743289507289532");
}
