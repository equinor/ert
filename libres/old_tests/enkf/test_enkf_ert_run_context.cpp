...
void test_create() {
    ...
    bool_vector_iset(iactive, 8, false);
    {
        ...
        path_fmt_type *runpath_fmt =
            path_fmt_alloc_directory_fmt("/tmp/path/%04d");
        ert_run_context_type *context =
            ert_run_context_alloc_ENSEMBLE_EXPERIMENT(
                fs, iactive, runpath_fmt, "job%d", subst_list, iter);
...

        {
            ...
            test_assert_string_equal("/tmp/path/0000",
                                     run_arg_get_runpath(run_arg0));
...
void test_create_ENSEMBLE_EXPERIMENT() {
    ...
    bool_vector_iset(iactive, 8, false);
    {
        subst_list_type *subst_list = subst_list_alloc(NULL);
        path_fmt_type *runpath_fmt =
            path_fmt_alloc_directory_fmt("/tmp/path/%04d/%d");
        enkf_fs_type *fs = NULL;
        ert_run_context_type *context =
            ert_run_context_alloc_ENSEMBLE_EXPERIMENT(fs, iactive, runpath_fmt,
                                                      "Job%d", subst_list, 7);
...

        {
            ...
            test_assert_string_equal("/tmp/path/0001/7",
                                     run_arg_get_runpath(run_arg1));

            test_assert_true(run_arg_is_instance(run_arg1));
        }

        for (int i = 0; i < ert_run_context_get_size(context); i++)
            test_assert_bool_equal(ert_run_context_iactive(context, i),
                                   bool_vector_iget(iactive, i));
...
void test_iactive_update() {
    ...
    enkf_fs_type *fs = NULL;
    bool_vector_type *iactive = bool_vector_alloc(10, true);
    ert_run_context_type *context = ert_run_context_alloc_ENSEMBLE_EXPERIMENT(
        fs, iactive, runpath_fmt, "Job%d", subst_list, 7);
...
void test_create_CASE_INIT() {
    ...
    ert_run_context_type *context =
        ert_run_context_alloc_CASE_INIT(sim_fs, iactive);
    ert_run_context_free(context);
    bool_vector_free(iactive);
}
