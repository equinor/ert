...
int main(int argc, char **argv) {
...

    {
        ...
        enkf_main = enkf_main_alloc(res_config);
        {
            ...
            enkf_config_node_type *field_config_node =
                ensemble_config_get_node(ens_config, "PORO");
            {
                char *init_file1 =
                    enkf_config_node_alloc_initfile(field_config_node, NULL, 0);
                char *init_file2 = enkf_config_node_alloc_initfile(
                    field_config_node, "/tmp", 0);

                test_assert_bool_equal(
                    enkf_config_node_use_forward_init(field_config_node),
                    forward_init);
            ...
            test_assert_bool_equal(enkf_node_use_forward_init(field_node),
                                   forward_init);
            if (forward_init)
                test_assert_bool_not_equal(
                    enkf_node_initialize(field_node, 0, rng), forward_init);
            // else hard_failure()
        ...
        if (forward_init) {
            ensemble_config_type *ens_config =
                enkf_main_get_ensemble_config(enkf_main);
            const enkf_config_node_type *field_config_node =
                ensemble_config_get_node(ens_config, "PORO");
            enkf_fs_type *fs = enkf_main_get_fs(enkf_main);
            enkf_node_type *field_node = enkf_node_alloc(field_config_node);
            ...
            util_copy_file(init_file, "simulations/run0/petro.grdecl");
            {
                ...
                {
                    double value;
                    test_assert_true(enkf_node_user_get(field_node, fs, "5,5,5",
                                                        node_id, &value));
            ...
            test_assert_true(
                enkf_node_fload(field_node, "simulations/run0/PORO.grdecl"));
            {
                double value;
                test_assert_true(enkf_node_user_get(field_node, fs, "4,4,4",
                                                    node_id, &value));
