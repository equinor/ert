...
int main(int argc, char **argv) {
...

    {
        ...
        test_assert_false(
            ecl_refcase_list_has_case(refcase_list, "DoesNotExist"));
        test_assert_NULL(
            ecl_refcase_list_get_case(refcase_list, "DoesNotExist"));

        test_assert_int_equal(
            ecl_refcase_list_add_matching(refcase_list, "DoesNotExist"), 0);
        ...
        test_assert_false(
            ecl_refcase_list_set_default(refcase_list, "DoesNotExist"));
        test_assert_false(ecl_refcase_list_has_default(refcase_list));
        test_assert_NULL(ecl_refcase_list_get_default(refcase_list));
...

    {
        ecl_refcase_list_type *refcase_list = ecl_refcase_list_alloc();
        test_assert_int_equal(
            ecl_refcase_list_add_matching(refcase_list, case_glob), 11);
        ...
        test_assert_int_equal(
            ecl_refcase_list_add_matching(refcase_list, case_glob), 0);
        test_assert_int_equal(
            ecl_refcase_list_add_matching(refcase_list, case_glob), 0);
        {
...

        {
            ...
            for (i = 0; i < N; i++)
                stringlist_append_copy(
                    case_list,
                    ecl_refcase_list_iget_pathcase(refcase_list, N - 1 - i));

            {
                bool equal = true;
                for (i = 0; i < N; i++)
                    equal = equal &&
                            util_string_equal(stringlist_iget(case_list, i),
                                              ecl_refcase_list_iget_pathcase(
                                                  refcase_list, i));
