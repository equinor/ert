...
void test_create() {
    ...
    test_assert_throw(row_scaling_iset(row_scaling, 0, -1),
                      std::invalid_argument);
    test_assert_throw(row_scaling_iset(row_scaling, 0, 2),
                      std::invalid_argument);
...
void row_scaling_multiply2(const row_scaling_type *row_scaling, matrix_type *A,
...
void test_multiply(const row_scaling_type *row_scaling, const matrix_type *A0,
                   const matrix_type *X0) {
...
void test_multiply() {
    ...
    // alpha == 1: Full update, should project out realizations project_iens
    {
        ...
        for (int row = 0; row < data_size; row++)
            for (int col = 0; col < ens_size; col++)
                test_assert_double_equal(matrix_iget(A, row, col),
                                         matrix_iget(A0, row, project_iens));
    ...
    // General alpha
    {
        ...
        test_assert_int_equal(row_scaling_get_size(row_scaling),
                              2 * data_size + 1);

        for (int row = 0; row < data_size; row++)
            row_data[row] = rng_get_double(rng);

        row_scaling_assign_double(row_scaling, row_data.data(),
                                  row_data.size());
        ...
        for (int row = 0; row < data_size; row++) {
            double alpha = row_scaling_iget(row_scaling, row);
            for (int col = 0; col < ens_size; col++) {
                double expected = alpha * matrix_iget(A0, row, project_iens) +
                                  (1 - alpha) * matrix_iget(A0, row, col);
