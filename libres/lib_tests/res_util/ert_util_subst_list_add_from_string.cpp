...
void test_valid_arg_string(const char *arg_string, int arg_count, ...)
{
    ...
    {
        test_assert_string_equal(subst_list_iget_key(subst_list, i), va_arg(ap, const char *));
        test_assert_string_equal(subst_list_iget_value(subst_list, i), va_arg(ap, const char *));
    }
    va_end(ap);
    subst_list_free(subst_list);
}
...
void test_invalid_arg_string(const char *arg_string)
{
    ...
    test_assert_util_abort("subst_list_add_from_string", call_func_util_abort, tmp);
    free(tmp);
}
