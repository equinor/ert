...
static const char *rms_comment2 =
    "Creator: RMS - Reservoir Modelling System, version 8.1";
...
rms_tag_type *rms_file_get_tag_ref(const rms_file_type *rms_file,
                                   const char *tagname, const char *keyname,
    ...
    for (int index = 0; index < size; ++index) {
        rms_tag_type *tag =
            (rms_tag_type *)vector_iget(rms_file->tag_list, index);
    ...
    if (return_tag == NULL && abort_on_error) {
        if (keyname != NULL && keyvalue != NULL)
            fprintf(stderr,
                    "%s: failed to find tag:%s with key:%s=%s in file:%s - "
                    "aborting \n",
                    __func__, tagname, keyname, keyvalue, rms_file->filename);
        else
            fprintf(stderr,
                    "%s: failed to find tag:%s in file:%s - aborting \n",
...
rms_file_type *rms_file_alloc(const char *filename, bool fmt_file) {
    ...
    hash_insert_hash_owned_ref(rms_file->type_map, "byte",
                               rms_type_alloc(rms_byte_type, 1), rms_type_free);
    hash_insert_hash_owned_ref(rms_file->type_map, "bool",
                               rms_type_alloc(rms_bool_type, 1), rms_type_free);
    hash_insert_hash_owned_ref(rms_file->type_map, "int",
                               rms_type_alloc(rms_int_type, 4), rms_type_free);
    hash_insert_hash_owned_ref(rms_file->type_map, "float",
                               rms_type_alloc(rms_float_type, 4),
                               rms_type_free);
    hash_insert_hash_owned_ref(rms_file->type_map, "double",
                               rms_type_alloc(rms_double_type, 8),
                               rms_type_free);

    hash_insert_hash_owned_ref(
        rms_file->type_map, "char", rms_type_alloc(rms_char_type, -1),
        rms_type_free); /* Char are a f*** mix of vector and scalar */
...
void rms_file_set_filename(rms_file_type *rms_file, const char *filename,
                           bool fmt_file) {
...
static void rms_file_init_fread(rms_file_type *rms_file) {
    ...
    {
        bool eof_tag;
        rms_tag_type *filedata_tag =
            rms_tag_fread_alloc(rms_file->stream, rms_file->type_map,
                                rms_file->endian_convert, &eof_tag);
        rms_tagkey_type *byteswap_key =
            rms_tag_get_key(filedata_tag, "byteswaptest");
        if (byteswap_key == NULL) {
            fprintf(stderr,
                    "%s: failed to find filedata/byteswaptest - aborting \n",
                    __func__);
            abort();
        }
...
rms_tag_type *rms_file_fread_alloc_tag(rms_file_type *rms_file,
    ...
    if (tag == NULL) {
        ...
        util_abort("%s: could not find tag: \"%s\" (with %s=%s) in file:%s - "
                   "aborting.\n",
                   __func__, tagname, keyname, keyvalue, rms_file->filename);
    }
...
rms_tagkey_type *rms_file_fread_alloc_data_tagkey(rms_file_type *rms_file,
                                                  ...
                                                  const char *keyvalue) {
    rms_tag_type *tag =
        rms_file_fread_alloc_tag(rms_file, tagname, keyname, keyvalue);
    if (tag == NULL)
        return NULL;
