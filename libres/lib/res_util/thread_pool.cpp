...
typedef struct {
    thread_pool_type
        *pool;      /* A back-reference to the thread_pool holding the queue. */
    ...
    void *
        func_arg; /* The arguments to this job - supplied by the calling scope. */
    start_func_ftype
        *func; /* The function to call - supplied by the calling scope. */
    void *return_value;
...
typedef struct {
    pthread_t
        thread; /* The thread variable currently (or more correct:last) running. */
...
struct thread_pool_struct {
    ...
    bool
        join; /* Flag set by the main thread to inform the dispatch thread that joining should start. */
    bool
        accepting_jobs; /* True|False whether the dispatch thread is running. */

    thread_pool_job_slot_type *
        job_slots; /* A vector to @max_running job slots, each slot can be reused several times.*/
...
*/
    ...
    static void thread_pool_resize_queue(thread_pool_type *pool,
                                         int queue_length) {
    pthread_rwlock_wrlock(&pool->queue_lock);
    {
        pool->queue = (thread_pool_arg_type *)util_realloc(
            pool->queue, queue_length * sizeof *pool->queue);
...
static void thread_pool_iset_return_value(thread_pool_type *pool, int index,
                                          void *return_value) {
...
static void *thread_pool_start_job(void *arg) {
    ...
    tp->job_slots[slot_index].running =
        false; /* We mark the job as completed. */
    free(arg);
...
static void *thread_pool_main_loop(void *arg) {
    thread_pool_type *tp = reinterpret_cast<thread_pool_type*>(arg);
    {
        ...
        int internal_offset =
            0; /* Keep track of the (index of) the last job slot fired off - minor time saving. */
        while (true) {
        ...
        */
                ...
                do {
                    int slot_index =
                        (counter + internal_offset) % tp->max_running;
                    thread_pool_job_slot_type *job_slot =
                        &tp->job_slots[slot_index];
                    if (!job_slot->running) {
            ...
            */
                        ...
                        tp_arg = (thread_pool_arg_type *)util_alloc_copy(
                            &tp->queue[tp->queue_index], sizeof *tp_arg);
            ...
            */
                        ...
                        pthread_create(&job_slot->thread, NULL,
                                       thread_pool_start_job, tp_arg);
                ...
                util_usleep(
                    usleep_init); /* There are no jobs wanting to run. */
...
void thread_pool_restart(thread_pool_type *tp) {
    if (tp->accepting_jobs)
        util_abort(
            "%s: fatal error - tried restart already running thread pool\n",
            __func__);
    {
        tp->join = false;
        tp->queue_index = 0;
...
void thread_pool_join(thread_pool_type *pool) {
    ...
    if (pool->max_running > 0) {
        pthread_join(pool->dispatch_thread,
                     NULL); /* Wait for the main thread to complete. */
...
bool thread_pool_try_join(thread_pool_type *pool, int timeout_seconds) {
    ...
    if (pool->max_running > 0) {
        ...
        {
            int join_return = pthread_timedjoin_np(
                pool->dispatch_thread, NULL,
                &ts); /* Wait for the main thread to complete. */
            if (join_return == 0)
                pool->accepting_jobs = false;
            else {
                pool->join = false;
...
void thread_pool_add_job(thread_pool_type *pool, start_func_ftype *start_func,
                         void *func_arg) {
    if (pool->max_running == 0) /* Blocking non-threaded mode: */
      ...
      */
            ...
            util_abort("%s: thread_pool is not running - restart with "
                       "thread_pool_restart()?? \n",
                       __func__);
    }
}
