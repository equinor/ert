...
void test_readwrite() {
    ecl::util::TestArea test_area("matrix_test");
    {
        ...
        test_assert_false(matrix_equal(m1, m2));
        {
            ...
            test_assert_int_equal(matrix_get_columns(m1),
                                  matrix_get_columns(m2));
...
void test_diag_std() {
    ...
    }
    {
        ...
        test_assert_double_equal(statistics_std(data),
                                 matrix_diag_std(m, mean));
        ...
        matrix_column_compressed_memcpy(m3, m1, mask);
        {
            ...
            for (src_col = 0; src_col < N; src_col++) {
                if (bool_vector_iget(mask, src_col)) {
                    test_assert_true(
                        matrix_columns_equal(m1, src_col, m3, target_col));
                    target_col++;
                }
            }
        }
