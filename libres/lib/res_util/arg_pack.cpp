...
typedef struct {
    void *
        buffer; /* This is the actual content - can either point to a remote object, or to storage managed by the arg_pack instance. */
    node_ctype ctype; /* The type of the data which is stored. */
    arg_node_free_ftype
        *destructor; /* destructor called on buffer - can be NULL. */
    arg_node_copyc_ftype
        *copyc; /* copy constructor - will typically be NULL. */
} arg_node_type;
...
struct arg_pack_struct {
    ...
    bool
        locked; /* To insure against unwaranted modifictaions - you can explicitly lock the arg_pack instance. This only */
...
static void __arg_node_assert_type(const arg_node_type *node,
                                   node_ctype arg_type) {
    if (arg_type != node->ctype)
        util_abort(
            "%s: asked for type:\'%s\'  inserted as:\'%s\'  - aborting \n",
...
static void *arg_node_get_ptr(const arg_node_type *node, bool get_ptr) {
    if (get_ptr) {
        if (node->ctype != CTYPE_VOID_POINTER)
            util_abort(
                "%s: tried to get pointer from something not a pointer\n",
                __func__);
    } else {
        if (node->ctype == CTYPE_VOID_POINTER)
            util_abort(
                "%s: tried to get adress to something already a ponter\n",
                __func__);
    }
    return node->buffer;
}
...
static void arg_node_set_ptr(arg_node_type *node, const void *ptr,
...
static void __arg_pack_assert_index(const arg_pack_type *arg, int iarg) {
    if (iarg < 0 || iarg >= arg->size)
        util_abort("%s: arg_pack() object filled with %d arguments - %d "
                   "invalid argument number - aborting \n",
                   __func__, arg->size, iarg);
}

static void arg_pack_realloc_nodes(arg_pack_type *arg_pack, int new_size) {
    arg_pack->nodes = (arg_node_type **)util_realloc(
        arg_pack->nodes, new_size * sizeof *arg_pack->nodes);
    {
        int i;
...
*/
static arg_node_type *arg_pack_iget_new_node(arg_pack_type *arg_pack,
                                             int index) {
                   ...
                   index, arg_pack->size);
    {
        if (index < arg_pack->size) {
            arg_node_free(
                arg_pack->nodes[index]); /* Free the existing current node. */
            arg_pack->nodes[index] =
                arg_node_alloc_empty(); /* Allocate a new fresh instance. */
        }

        if (arg_pack->size == arg_pack->alloc_size)
            arg_pack_realloc_nodes(
                arg_pack, 1 + arg_pack->alloc_size *
                                  2); /* We have to grow the vector of nodes. */

        if (index == arg_pack->size)
            arg_pack
                ->size++; /* We are asking for the first element beyond the current length of the vector, i.e. append. */
...
static arg_node_type *arg_pack_get_append_node(arg_pack_type *arg_pack) {
    ...
    {
        arg_node_type *new_node =
            arg_pack_iget_new_node(arg_pack, arg_pack->size);
        return new_node;
    }
}
...
void arg_pack_iset_copy(arg_pack_type *arg_pack, int index, const void *ptr,
                        arg_node_copyc_ftype *copyc,
                        arg_node_free_ftype *freef) {
...
void arg_pack_iset_owned_ptr(arg_pack_type *arg_pack, int index, void *ptr,
                             arg_node_free_ftype *freef) {
...
void arg_pack_append_owned_ptr(arg_pack_type *arg_pack, void *ptr,
                               arg_node_free_ftype *freef) {
    arg_pack_iset_owned_ptr(arg_pack, arg_pack->size, ptr, freef);
}

int arg_pack_size(const arg_pack_type *arg_pack) { return arg_pack->size; }
