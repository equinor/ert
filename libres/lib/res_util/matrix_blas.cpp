...
void dgemv_(char *, int *, int *, double *, double *, int *, const double *,
...
void matrix_dgemv(const matrix_type *A, const double *x, double *y, bool transA,
    ...
    dgemv_(&transA_c, &m, &n, &alpha, matrix_get_data(A), &lda, x, &incx, &beta,
           y, &incy);
}

static void dgemm_debug(const matrix_type *C, const matrix_type *A,
                        const matrix_type *B, bool transA, bool transB) {
...
void matrix_dgemm(matrix_type *C, const matrix_type *A, const matrix_type *B,
  ...
  */
    ...
    if (ldc < util_int_max(1, m)) {
        dgemm_debug(C, A, B, transA, transB);
        fprintf(stderr, "Tried to capture blas message: \"** On entry to DGEMM "
                        "parameter 13 had an illegal value\"\n");
