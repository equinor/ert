...
const char *log_get_filename(const log_type *logh) { return logh->filename; }

int log_get_msg_count(const log_type *logh) { return logh->msg_count; }
...
static bool log_include_message(const log_type *logh,
                                message_level_type message_level) {
    ...
    if (add_timestamp) {
        if (message)
            ...
            fprintf(stream, "%02d/%02d - %02d:%02d:%02d   \n",
                    time_fields.tm_mday, time_fields.tm_mon + 1,
                    time_fields.tm_hour, time_fields.tm_min,
                    time_fields.tm_sec);
    } else if (message)
        fprintf(stream, "%s\n", message);
}
...
void log_add_message(log_type *logh, message_level_type message_level,
    ...
    if (logh->stream == NULL)
        util_abort("%s: logh->stream == NULL - must call log_reset_filename() "
                   "first \n",
                   __func__);
...
void log_close(log_type *logh) {
    if (logh->stream_owner)
        fclose(
            logh->stream); /* This closes BOTH the FILE * stream and the integer file descriptor. */
