...
static void res_log_init_stream(message_level_type log_level, FILE *stream) {
    if (logh)
        log_close(logh);
...
void res_log_add_message(message_level_type message_level,
    ...
    if (!logh) {
        if (message_level >= LOG_ERROR)
            log_add_message_stream(stderr, message_level,
                                   (message_level_type) false, message);
        return;
    }
...
static void res_log_va(message_level_type level, const char *fmt,
                       va_list args) {
...
void res_log_debug(const char *msg) { res_log_add_message(LOG_DEBUG, msg); }

void res_log_info(const char *msg) { res_log_add_message(LOG_INFO, msg); }

void res_log_warning(const char *msg) { res_log_add_message(LOG_WARNING, msg); }

void res_log_error(const char *msg) { res_log_add_message(LOG_ERROR, msg); }
