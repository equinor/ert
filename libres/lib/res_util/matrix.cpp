...
static void matrix_init_header(matrix_type *matrix, int rows, int columns,
                               int row_stride, int column_stride) {

    if (!((column_stride * columns <= row_stride) ||
          (row_stride * rows <= column_stride)))
...
static void matrix_realloc_data__(matrix_type *matrix, bool safe_mode) {
    if (matrix->data_owner) {
      ...
      */
            ...
            matrix->data =
                (double *)util_malloc(sizeof *matrix->data * data_size);
...
/*
  ...
  allocation fails.
*/
static matrix_type *matrix_alloc_with_stride(int rows, int columns,
                                             int row_stride, int column_stride,
                                             bool safe_mode) {
...
matrix_type *matrix_alloc_shared(const matrix_type *src, int row, int column,
    ...
    if (((row + rows) > src->rows) || ((column + columns) > src->columns))
        util_abort("%s: Invalid matrix subsection src:[%d,%d]  Offset:[%d,%d]  "
                   "SubSize:[%d,%d] \n",
                   __func__, src->rows, src->columns, row, column, rows,
                   columns);

    {
        matrix_type *matrix = matrix_alloc_empty();

        matrix_init_header(matrix, rows, columns, src->row_stride,
                           src->column_stride);
...
static matrix_type *matrix_alloc__(int rows, int columns, bool safe_mode) {
    return matrix_alloc_with_stride(
        rows, columns, 1, rows,
        safe_mode); /* Must be the stride (1,rows) to use the lapack routines. */
}
...
/*
   ...
   layout.
*/
static matrix_type *matrix_alloc_copy__(const matrix_type *src,
                                        bool safe_mode) {
    matrix_type *copy = matrix_alloc__(matrix_get_rows(src),
                                       matrix_get_columns(src), safe_mode);
    if (copy != NULL)
...
matrix_type *matrix_alloc_column_compressed_copy(const matrix_type *src,
    ...
    if (bool_vector_size(mask) != matrix_get_columns(src))
        util_abort(
            "%s: size mismatch. Src matrix has %d rows  mask has:%d elements\n",
            __func__, matrix_get_rows(src), bool_vector_size(mask));
    {
        int target_columns = bool_vector_count_equal(mask, true);
        matrix_type *target =
            matrix_alloc(matrix_get_rows(src), target_columns);
...
void matrix_copy_block(matrix_type *target_matrix, int target_row,
                       ...
                       int src_column) {
    matrix_type *target_view = matrix_alloc_shared(
        target_matrix, target_row, target_column, rows, columns);
    matrix_type *src_view =
        matrix_alloc_shared(src_matrix, src_row, src_column, rows, columns);
...
matrix_type *matrix_alloc_sub_copy(const matrix_type *src, int row_offset,
    ...
    matrix_copy_block(copy, 0, 0, rows, columns, src, row_offset,
                      column_offset);
    return copy;
}

static bool matrix_resize__(matrix_type *matrix, int rows, int columns,
                            bool copy_content, bool safe_mode) {
    if (!matrix->data_owner)
        util_abort("%s: sorry - can not resize shared matrizes. \n", __func__);
    {
        ...
        if ((rows != matrix->rows) || (columns != matrix->columns)) {
            ...
            if (copy_content) {
                copy_view = matrix_alloc_shared(
                    matrix, 0, 0, copy_rows,
                    copy_columns); /* This is the part of the old matrix which should be copied over to the new. */
                copy = matrix_alloc_copy__(
                    copy_view,
                    safe_mode); /* Now copy contains the part of the old matrix which should be copied over - with private storage. */
            }
            {
                int old_rows, old_columns, old_row_stride, old_column_stride;
                matrix_get_dims(
                    matrix, &old_rows, &old_columns, &old_row_stride,
                    &old_column_stride); /* Storing the old header information - in case the realloc() fails. */

                matrix_init_header(
                    matrix, rows, columns, 1,
                    rows); /* Resetting the header for the matrix */
                matrix_realloc_data__(matrix, safe_mode);
                if (matrix->data != NULL) { /* Realloc succeeded */
                    if (copy_content) {
                        matrix_type *target_view = matrix_alloc_shared(
                            matrix, 0, 0, copy_rows, copy_columns);
                        matrix_assign(target_view, copy);
                        matrix_free(target_view);
                    }
                } else {
                    /* Failed to realloc new storage; RETURNING AN INVALID MATRIX */
                    matrix_init_header(matrix, old_rows, old_columns,
                                       old_row_stride, old_column_stride);
                    resize_OK = false;
                }
            }
...
void matrix_pretty_fprint_submat(const matrix_type *matrix, const char *name,
    ...
    if (m < 0 || m > M || M >= matrix->rows || n < 0 || n > N ||
        N >= matrix->columns)
...
void matrix_fwrite(const matrix_type *matrix, FILE *stream) {
    ...
    if (matrix->column_stride == matrix->rows)
        util_fwrite(matrix->data, sizeof *matrix->data,
                    matrix->columns * matrix->rows, stream, __func__);
    else {
        int column;
        for (column = 0; column < matrix->columns; column++) {
            if (matrix->row_stride == 1) {
                const double *column_data =
                    &matrix->data[column * matrix->column_stride];
                util_fwrite(column_data, sizeof *column_data, matrix->rows,
                            stream, __func__);
            } else {
                int row;
                for (row = 0; row < matrix->rows; row++)
                    util_fwrite_double(
                        matrix->data[GET_INDEX(matrix, row, column)], stream);
            }
        }
    }
}
...
void matrix_fread(matrix_type *matrix, FILE *stream) {
    ...
    if (matrix->column_stride == matrix->rows)
        util_fread(matrix->data, sizeof *matrix->data,
                   matrix->columns * matrix->rows, stream, __func__);
    else {
        int column;
        for (column = 0; column < matrix->columns; column++) {
            if (matrix->row_stride == 1) {
                double *column_data =
                    &matrix->data[column * matrix->column_stride];
                util_fread(column_data, sizeof *column_data, matrix->rows,
                           stream, __func__);
            } else {
                int row;
                for (row = 0; row < matrix->rows; row++)
                    matrix->data[GET_INDEX(matrix, row, column)] =
                        util_fread_double(stream);
            }
        }
    }
}
...
void matrix_set_many_on_column(matrix_type *matrix, int row_offset,
    ...
    if ((row_offset + elements) <= matrix->rows) {
        if (matrix->row_stride == 1) /* Memory is continous ... */
            memcpy(&matrix->data[GET_INDEX(matrix, row_offset, column)], data,
                   elements * sizeof *data);
        else {
            int i;
            for (i = 0; i < elements; i++)
                matrix->data[row_offset + GET_INDEX(matrix, i, column)] =
                    data[i];
        }
    } else
...
void matrix_copy_column(matrix_type *target_matrix,
                        const matrix_type *src_matrix, int target_column,
                        int src_column) {
    matrix_assert_equal_rows(target_matrix, src_matrix);
    {
        int row;
        for (row = 0; row < target_matrix->rows; row++)
            target_matrix->data[GET_INDEX(target_matrix, row, target_column)] =
                src_matrix->data[GET_INDEX(src_matrix, row, src_column)];
    }
}
...
void matrix_copy_row(matrix_type *target_matrix, const matrix_type *src_matrix,
    ...
    matrix_assert_equal_columns(target_matrix, src_matrix);
    {
        ...
        for (col = 0; col < target_matrix->columns; col++)
            target_matrix->data[GET_INDEX(target_matrix, target_row, col)] =
                src_matrix->data[GET_INDEX(src_matrix, src_row, col)];
    }
}
...
double matrix_column_column_dot_product(const matrix_type *m1, int col1,
        ...
        util_abort("%s: size mismatch \n", __func__);
    {
        ...
        for (row = 0; row < m1->rows; row++)
            sum += m1->data[GET_INDEX(m1, row, col1)] *
                   m2->data[GET_INDEX(m2, row, col2)];
...
double matrix_row_column_dot_product(const matrix_type *m1, int row1,
    ...
    if (m1->columns != m2->rows)
        util_abort("%s: size mismatch: m1:[%d,%d]   m2:[%d,%d] \n", __func__,
                   matrix_get_rows(m1), matrix_get_columns(m1),
                   matrix_get_rows(m2), matrix_get_columns(m2));

    {
        int k;
        double sum = 0;
        for (k = 0; k < m1->columns; k++)
            sum += m1->data[GET_INDEX(m1, row1, k)] *
                   m2->data[GET_INDEX(m2, k, col2)];
...
void matrix_assign(matrix_type *A, const matrix_type *B) {
    if ((A->rows == B->rows) && (A->columns == B->columns)) {
        ...
        if (A->row_stride == B->row_stride) {
            if (A->columns ==
                A->row_stride) /* Memory is just one continous block */
                memcpy(A->data, B->data,
                       A->rows * A->columns * sizeof *A->data);
            else {
                /* Copying columns of data */
                for (j = 0; j < A->columns; j++)
                    memcpy(&A->data[GET_INDEX(A, 0, j)],
                           &B->data[GET_INDEX(B, 0, j)],
                           A->rows * sizeof *A->data);
            }
        } else {
...
void matrix_inplace_sub_column(matrix_type *A, const matrix_type *B, int colA,
                               int colB) {
    if ((A->rows == B->rows) && (colA < A->columns) && (colB < B->columns)) {
        int row;

        for (row = 0; row < A->rows; row++)
            A->data[GET_INDEX(A, row, colA)] -=
                B->data[GET_INDEX(B, row, colB)];
...
void matrix_sub(matrix_type *A, const matrix_type *B, const matrix_type *C) {
    if ((A->rows == B->rows) && (A->columns == B->columns) &&
        (A->rows == C->rows) && (A->columns == C->columns)) {
        int i, j;

        for (j = 0; j < A->columns; j++)
            for (i = 0; i < A->rows; i++)
                A->data[GET_INDEX(A, i, j)] =
                    B->data[GET_INDEX(B, i, j)] - C->data[GET_INDEX(B, i, j)];
        ...
        for (i = 0; i < A->rows; i++) {
            ...
            for (j = 0; j < B->rows; j++) {
                ...
                for (k = 0; k < A->columns; k++)
                    scalar_product += A->data[GET_INDEX(A, i, k)] *
                                      B->data[GET_INDEX(B, k, j)];
...
static void *matrix_inplace_matmul_mt__(void *arg) {
    ...
    const matrix_type *B =
        (const matrix_type *)arg_pack_iget_const_ptr(arg_pack, 3);

    matrix_type *A_view =
        matrix_alloc_shared(A, row_offset, 0, rows, matrix_get_columns(A));
...
void matrix_inplace_matmul_mt2(matrix_type *A, const matrix_type *B,
    ...
    arg_pack_type **arglist =
        (arg_pack_type **)util_malloc(num_threads * sizeof *arglist);
    int it;
    thread_pool_restart(thread_pool);
    {
        ...
        for (it = 0; it < num_threads; it++) {
            ...
            thread_pool_add_job(thread_pool, matrix_inplace_matmul_mt__,
                                arglist[it]);
            row_offset += row_size;
        }
    }
...
double *matrix_get_data(const matrix_type *matrix) { return matrix->data; }
...
int matrix_get_columns(const matrix_type *matrix) { return matrix->columns; }
    ...
    for (i = 0; i < matrix->rows; i++)
        for (j = 0; j < matrix->columns; j++)
            if (!std::isfinite(matrix->data[GET_INDEX(matrix, i, j)])) {
                printf("%s(%d,%d) = %g \n", matrix->name, i, j,
                       matrix->data[GET_INDEX(matrix, i, j)]);
                return false;
            }
...
bool matrix_columns_equal(const matrix_type *m1, int col1,
...

    {
        int row;
        for (row = 0; row < m1->rows; row++) {
            if (memcmp(&m1->data[GET_INDEX(m1, row, col1)],
                       &m2->data[GET_INDEX(m2, row, col2)],
                       sizeof *m1->data) != 0)
                return false;
        }
...
bool matrix_check_dims(const matrix_type *m, int rows, int columns) {
    if (m) {
        ...
        util_abort(
            "%s: internal error - trying to dereference NULL matrix pointer \n",
            __func__);
        return false;
    }
}
