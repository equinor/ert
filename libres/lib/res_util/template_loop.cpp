...
#define LOOP_REGEXP                                                            \
    "[{]%[[:space:]]+for[[:space:]]+([$]?[[:alpha:]][[:alnum:]]*)[[:space:]]+" \
    "in[[:space:]]+[[]([^]]*)[]][[:space:]]+%[}]"
...
static loop_type *loop_alloc(const char *buffer, int global_offset,
                             regmatch_t tag_offset, regmatch_t __var_offset,
                             regmatch_t __items_offset) {
    ...
    loop->body_length = -1;
    {
        ...
        loop->loop_var =
            util_alloc_substring_copy(buffer, var_offset, var_length);
    ...
    }
    {
        ...
        char *items_string =
            util_alloc_substring_copy(buffer, items_offset, items_length);
        basic_parser_type *parser = basic_parser_alloc(
            ",", NULL, NULL, NULL, NULL,
            NULL); // Does not tolerate spaces around the splitting ","
...
static void loop_set_endmatch(loop_type *loop, int global_offset,
                              regmatch_t end_offset) {
    loop->endtag_offset = global_offset + end_offset.rm_so;
    loop->endtag_length = end_offset.rm_eo - end_offset.rm_so;
    loop->body_length =
        loop->endtag_offset - loop->opentag_offset - loop->opentag_length;
}
...
static void replace_1var(buffer_type *var_expansion, int shift,
                         int write_offset, int shift_offset,
                         const char *value) {
...
static void loop_eval(const loop_type *loop, const char *body,
                      buffer_type *var_expansion, int ivar) {
        ...
        */
             ...
             alternatively followed by a NON alphanumeric character. */
                ...
                /* OK - this is a valid match; update the string buffer. */
                {
                    ...
                    replace_1var(var_expansion, shift, write_offset,
                                 shift_offset, value);
...
int template_eval_loop(const template_type *_template, buffer_type *buffer,
                       int global_offset, loop_type *loop) {
    int flags = 0;
    int NMATCH = 3;
    ...
    int search_offset = loop->opentag_offset + loop->opentag_length;
    {
        ...
        {
            char *search_data = (char *)buffer_get_data(
                buffer); // This pointer must be refetched because the buffer can realloc and move it.
            loop_match =
                regexec(&_template->start_regexp, &search_data[search_offset],
                        NMATCH, match_list_loop, flags);
            end_match =
                regexec(&_template->end_regexp, &search_data[search_offset],
                        NMATCH, match_list_end, flags);
        }
        ...
        if (loop_match == 0) {
            if (match_list_loop[0].rm_so < match_list_end[0].rm_so) {
                ...
                loop_type *sub_loop =
                    loop_alloc(search_data, search_offset, match_list_loop[0],
                               match_list_loop[1], match_list_loop[2]);
                global_offset = template_eval_loop(_template, buffer,
                                                   global_offset, sub_loop);
            }
        }
       ...
       offset. */
        ...
        {
            buffer_type *loop_expansion = buffer_alloc(
                loop->body_length * stringlist_get_size(loop->items));
            {
                char *src_data = (char *)buffer_get_data(buffer);
                char *body = util_alloc_substring_copy(
                    src_data, loop->body_offset, loop->body_length);
                buffer_type *var_expansion = buffer_alloc(0);
                int ivar;

                for (ivar = 0; ivar < stringlist_get_size(loop->items);
                     ivar++) {
                    loop_eval(loop, body, var_expansion, ivar);
                    buffer_strcat(loop_expansion,
                                  (const char *)buffer_get_data(var_expansion));
                }
            ...
            {
                int tag_length = loop->endtag_offset + loop->endtag_length -
                                 loop->opentag_offset;
                ...
                buffer_fwrite(buffer, buffer_get_data(loop_expansion), 1,
                              buffer_get_string_size(loop_expansion));

                global_offset = loop->opentag_offset +
                                buffer_get_string_size(loop_expansion);
            }
...
void template_eval_loops(const template_type *_template, buffer_type *buffer) {
    ...
    regmatch_t match_list[NMATCH];
    {
        ...
        while (true) {
            char *src_data = (char *)buffer_get_data(buffer);
            int match =
                regexec(&_template->start_regexp, &src_data[global_offset],
                        NMATCH, match_list, 0);
            if (match == 0) {
                loop_type *loop =
                    loop_alloc(src_data, global_offset, match_list[0],
                               match_list[1], match_list[2]);
                global_offset =
                    template_eval_loop(_template, buffer, global_offset, loop);
                if (global_offset < 0) {
                    fprintf(stderr,
                            "** Warning ** : found {%% for .... %%} loop "
                            "construct without mathcing {%% endfor %%}\n");
                    break;
                }
