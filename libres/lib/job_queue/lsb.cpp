...
static void *lsb_dlsym(lsb_type *lsb, const char *function_name) {
    ...
    if (!function) {
        ...
        stringlist_append_owned_ref(
            lsb->error_list,
            util_alloc_sprintf("Failed to locate symbol:%s  dlerror:%s",
...
void *lsb_dlopen(lsb_type *lsb, const char *lib_name) {
    ...
    if (!lib_handle) {
        lsb->ready = false;
        stringlist_append_owned_ref(
            lsb->error_list, util_alloc_sprintf("dlopen(%s) - failed:%s \n",
                                                lib_name, dlerror()));
    }
    return lib_handle;
}
...
lsb_type *lsb_alloc() {
    ...
    if (lsb->lib_bat) {
        ...
        lsb->open_job =
            (lsb_openjobinfo_ftype *)lsb_dlsym(lsb, "lsb_openjobinfo");
        lsb->read_job =
            (lsb_readjobinfo_ftype *)lsb_dlsym(lsb, "lsb_readjobinfo");
        lsb->close_job =
            (lsb_closejobinfo_ftype *)lsb_dlsym(lsb, "lsb_closejobinfo");
        lsb->kill_job =
            (lsb_forcekilljob_ftype *)lsb_dlsym(lsb, "lsb_forcekilljob");
...
int lsb_submitjob(const lsb_type *lsb, struct submit *submit_data,
                  struct submitReply *reply_data) {
...
int lsb_closejob(const lsb_type *lsb) { return lsb->close_job(); }

char *lsb_sys_msg(const lsb_type *lsb) { return lsb->sys_msg(); }
