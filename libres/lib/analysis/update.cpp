...
void serialize_ministep(const ensemble_config_type *ens_config,
    ...
    for (auto &key : ministep->unscaled_keys()) {
        ...
        int matrix_rows = A.rows();
        if ((active_size + current_row) > matrix_rows)
            A.conservativeResize(matrix_rows + 2 * active_size, ens_size);
        if (active_size > 0) {
...
void run_analysis_update_without_rowscaling(
    ...
    int active_obs_size = S.rows();
    Eigen::MatrixXd X = Eigen::MatrixXd::Zero(active_ens_size, active_ens_size);
