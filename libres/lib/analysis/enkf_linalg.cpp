...
void enkf_linalg_rml_enkfX1(matrix_type *X1, matrix_type *Udr, matrix_type *D,
                            matrix_type *R) {
    /*
  ...
  */
    matrix_type *tmp =
        matrix_alloc(matrix_get_columns(Udr), matrix_get_rows(R));
...
double enkf_linalg_data_mismatch(matrix_type *D, matrix_type *R,
                                 matrix_type *Sk) {
    matrix_type *tmp =
        matrix_alloc(matrix_get_columns(D), matrix_get_columns(R));
    double mismatch;
    matrix_matmul_with_transpose(tmp, D, R, true,
                                 false); // tmp = D' * R, i.e. N-by-p
