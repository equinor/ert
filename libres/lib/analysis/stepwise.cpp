...
static double stepwise_estimate__(stepwise_type *stepwise,
                                  bool_vector_type *active_rows) {
    matrix_type *X;
    matrix_type *E;
    matrix_type *Y;
    ...
    matrix_set(
        stepwise->beta,
        0); // It is essential to make sure that old finite values in the beta0 vector do not hang around.
  ...
  */
                ...
                if (bool_vector_iget(active_rows, irow)) {
                    ...
                    for (icol = 0; icol < ncols; icol++) {
                        if (bool_vector_iget(stepwise->active_set, icol)) {
                            matrix_iset(X, arow, acol,
                                        matrix_iget(stepwise->X0, irow, icol));
                            matrix_iset(E, arow, acol,
                                        matrix_iget(stepwise->E0, irow, icol));
                            acol++;
                        }
                    }
...

    {
        ...
        matrix_type *beta = matrix_alloc(
            nvar,
            1); /* This is the beta vector as estimated from the OLS estimator. */
    ...
    */
        {
            ...
            for (ivar = 0; ivar < ncols; ivar++) {
                if (bool_vector_iget(stepwise->active_set, ivar)) {
                    matrix_iset(stepwise->beta, ivar, 0,
                                matrix_iget(beta, avar, 0));
                    avar++;
                }
            }
        }
...
static double stepwise_eval__(const stepwise_type *stepwise,
                              const matrix_type *x) {
    return matrix_row_column_dot_product(x, 0, stepwise->beta, 0);
}

static double stepwise_test_var(stepwise_type *stepwise, int test_var,
                                int blocks) {
    double prediction_error = 0;

    bool_vector_iset(stepwise->active_set, test_var,
                     true); // Temporarily activate this variable
    {
        ...
        for (int iblock = 0; iblock < blocks; iblock++) {
             ...
             */
            {
                stepwise_estimate__(stepwise, active_rows);
                {
                    ...
                    for (irow = validation_start; irow <= validation_end;
                         irow++) {
                        matrix_copy_row(x_vector, stepwise->X0, 0,
                                        randperms[irow]);
                        {
                            double true_value =
                                matrix_iget(stepwise->Y0, randperms[irow], 0);
                            double estimated_value =
                                stepwise_eval__(stepwise, x_vector);
                            prediction_error += (true_value - estimated_value) *
...
void stepwise_estimate(stepwise_type *stepwise, double deltaR2_limit,
    ...
    while (true) {
...

        {
            ...
            if (bool_vector_count_equal(stepwise->active_set, true) ==
                matrix_get_columns(stepwise->X0)) {
                stepwise_estimate__(stepwise, active_rows);
...
stepwise_type *stepwise_alloc1(int nsample, int nvar, rng_type *rng,
                               const matrix_type *St, const matrix_type *Et) {
    stepwise_type *stepwise = stepwise_alloc__(nsample, nvar, rng);

    stepwise->rng = rng;
    stepwise->X0 = matrix_alloc_copy(
        St); // It would be nice to get rid of these copies, but due to data race it is not possible at the moment
