...
static UTIL_SAFE_CAST_FUNCTION(
    sqrt_enkf_data,
    SQRT_ENKF_TYPE_ID) static UTIL_SAFE_CAST_FUNCTION_CONST(sqrt_enkf_data,
                                                            SQRT_ENKF_TYPE_ID)

    void *sqrt_enkf_data_alloc() {
    sqrt_enkf_data_type *data =
        (sqrt_enkf_data_type *)util_malloc(sizeof *data);
...
void sqrt_enkf_initX(void *module_data, matrix_type *X, const matrix_type *A,
...
void sqrt_enkf_init_update(void *arg, const bool_vector_type *ens_mask,
...
bool sqrt_enkf_has_var(const void *arg, const char *var_name) {
    const sqrt_enkf_data_type *module_data =
        sqrt_enkf_data_safe_cast_const(arg);
