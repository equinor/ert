...
static UTIL_SAFE_CAST_FUNCTION(
    bootstrap_enkf_data,
    BOOTSTRAP_ENKF_TYPE_ID) static UTIL_SAFE_CAST_FUNCTION_CONST(bootstrap_enkf_data,
                                                                 BOOTSTRAP_ENKF_TYPE_ID)

    void bootstrap_enkf_set_doCV(bootstrap_enkf_data_type *data, bool doCV) {
    data->doCV = doCV;
}

void bootstrap_enkf_set_truncation(bootstrap_enkf_data_type *boot_data,
                                   double truncation) {
    std_enkf_set_truncation(boot_data->std_enkf_data, truncation);
    cv_enkf_set_truncation(boot_data->cv_enkf_data, truncation);
}

void bootstrap_enkf_set_subspace_dimension(bootstrap_enkf_data_type *boot_data,
                                           int ncomp) {
...
void *bootstrap_enkf_data_alloc() {
    bootstrap_enkf_data_type *boot_data =
        (bootstrap_enkf_data_type *)util_malloc(sizeof *boot_data);
...
static int **alloc_iens_resample(rng_type *rng, int ens_size) {
    ...
    for (iens = 0; iens < ens_size; iens++)
        iens_resample[iens] =
            (int *)util_calloc(ens_size, sizeof(**iens_resample));
...
void bootstrap_enkf_updateA(void *module_data, matrix_type *A,
    ...
    bootstrap_enkf_data_type *bootstrap_data =
        bootstrap_enkf_data_safe_cast(module_data);
    {
        ...
        matrix_type *A_resampled =
            matrix_alloc(matrix_get_rows(A0), matrix_get_columns(A0));
        int **iens_resample = alloc_iens_resample(rng, ens_size);
        {
            int ensemble_members_loop;
            for (ensemble_members_loop = 0; ensemble_members_loop < ens_size;
                 ensemble_members_loop++) {
                int ensemble_counter;
                /* Resample A and meas_data. Here we are careful to resample the working copy.*/
                {
                    {
                        int_vector_type *bootstrap_components =
                            int_vector_alloc(ens_size, 0);
                        for (ensemble_counter = 0; ensemble_counter < ens_size;
                             ensemble_counter++) {
                            int random_column =
                                iens_resample[ensemble_members_loop]
                                             [ensemble_counter];
                            int_vector_iset(bootstrap_components,
                                            ensemble_counter, random_column);
                            matrix_copy_column(A_resampled, A0,
                                               ensemble_counter, random_column);
                            matrix_copy_column(S_resampled, S, ensemble_counter,
                                               random_column);
                        }
                    ...
                    if (bootstrap_data->doCV) {
                        ...
                        cv_enkf_init_update(bootstrap_data->cv_enkf_data,
                                            ens_mask, obs_mask, S_resampled, R,
                                            dObs, E, D, rng);
                        cv_enkf_initX(bootstrap_data->cv_enkf_data, X,
                                      A_resampled, S_resampled, R, dObs, E, D,
                                      rng);
                    } else
                        std_enkf_initX(bootstrap_data->std_enkf_data, X, NULL,
                                       S_resampled, R, dObs, E, D, rng);

                    matrix_inplace_matmul_mt1(A_resampled, X, num_cpu_threads);
                    matrix_inplace_add(A_resampled, A0);
                    matrix_copy_column(A, A_resampled, ensemble_members_loop,
                                       ensemble_members_loop);
                }
            }
        }
...
long bootstrap_enkf_get_options(void *arg, long flag) {
    bootstrap_enkf_data_type *bootstrap_data =
        bootstrap_enkf_data_safe_cast(arg);
...
bool bootstrap_enkf_has_var(const void *arg, const char *var_name) {
    const bootstrap_enkf_data_type *module_data =
        bootstrap_enkf_data_safe_cast_const(arg);
