...
static UTIL_SAFE_CAST_FUNCTION_CONST(
    std_enkf_debug_data,
    STD_ENKF_DEBUG_TYPE_ID) static UTIL_SAFE_CAST_FUNCTION(std_enkf_debug_data,
                                                           STD_ENKF_DEBUG_TYPE_ID)

    void std_enkf_debug_data_set_prefix(std_enkf_debug_data_type *data,
                                        const char *prefix) {
...
void std_enkf_debug_save_matrix(const matrix_type *m, const char *path,
                                const char *file, bool transpose) {
...
void std_enkf_debug_initX(void *module_data, matrix_type *X, matrix_type *A,
                          matrix_type *S, matrix_type *R, matrix_type *dObs,
                          matrix_type *E, matrix_type *D, rng_type *rng) {

    std_enkf_debug_data_type *data = std_enkf_debug_data_safe_cast(module_data);
    char *debug_path =
        util_alloc_sprintf("%s/%d", data->prefix, data->update_count);
    ...
    {
        matrix_type *value =
            matrix_alloc_sub_copy(dObs, 0, 0, matrix_get_rows(dObs), 1);
        matrix_type *std =
            matrix_alloc_sub_copy(dObs, 0, 1, matrix_get_rows(dObs), 1);

        std_enkf_debug_save_matrix(value, debug_path, "observations.csv", true);
        std_enkf_debug_save_matrix(std, debug_path, "observations_std.csv",
                                   true);
    ...
    std_enkf_initX(std_data, X, A, S, R, dObs, E, D, rng);
    {
        ...
        std_enkf_debug_save_matrix(posterior, debug_path, "posterior_ert.csv",
                                   true);
...
bool std_enkf_debug_set_string(void *arg, const char *var_name,
                               const char *value) {
    std_enkf_debug_data_type *module_data = std_enkf_debug_data_safe_cast(arg);
...
long std_enkf_debug_get_options(void *arg, long flag) {
    const std_enkf_debug_data_type *module_data =
        std_enkf_debug_data_safe_cast_const(arg);
