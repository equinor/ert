...
#include <ert/util/matrix_blas.hpp>
...
struct rml_enkf_data_struct {
    ...
    double
        lambda; // parameter to control the search direction in Marquardt levenberg optimization
...
void rml_enkf_set_subspace_dimension(rml_enkf_data_type *data,
...
void rml_enkf_set_iteration_number(rml_enkf_data_type *data,
...
void *rml_enkf_data_alloc(rng_type *rng) {
    ...
    data->option_flags = ANALYSIS_NEED_ED + ANALYSIS_UPDATE_A +
                         ANALYSIS_ITERABLE + ANALYSIS_SCALE_DATA;
    data->iteration_nr = 0;
    data->Std = 0;
    data->state = matrix_alloc(
        1, 1); // This will be resized under use; but we need a valid instance
    data->lambda0 = -1.0;
...
void rml_enkf_updateA(void *module_data, matrix_type *A, matrix_type *S,
    ...
    matrix_type *Skm =
        matrix_alloc(matrix_get_columns(D), matrix_get_columns(D));
    ...
    if (data->iteration_nr == 0) {
        Sk_new = enkf_linalg_data_mismatch(
            D, Cd, Skm); //Calculate the intitial data mismatch term
        ...
        rml_enkf_common_initA__(A, S, Cd, E, D, truncation, data->lambda, Ud,
                                Wd, VdT);
        data->Sk = Sk_new;
        data->Std = Std_new;
        printf("Prior Objective function value is %5.3f \n", data->Sk);

        fprintf(fp, "Iteration number\t   Lamda Value \t    Current Mean (OB "
                    "FN) \t    Old Mean\t     Current Stddev\n");
        fprintf(fp, "\n\n");
        fprintf(fp,
                "%d     \t\t       NA       \t      %5.5f      \t         \t   "
                "%5.5f    \n",
        ...
        if ((Sk_new < (data->Sk)) && (Std_new < (data->Std))) {
            if ((1 - (Sk_new / data->Sk)) <
                .0001) // check convergence ** model change norm has to be added in this!!
                data->iteration_nr = 16;

            fprintf(fp,
                    "%d     \t\t      %5.5f      \t      %5.5f      \t    "
                    "%5.5f    \t   %5.5f    \n",
                    data->iteration_nr, data->lambda, Sk_new, data->Sk,
                    Std_new);
            ...
            data->Sk = Sk_new;
            rml_enkf_common_initA__(A, S, Cd, E, D, truncation, data->lambda,
                                    Ud, Wd, VdT);
