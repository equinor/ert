...
struct rml_enkf_data_struct {
    ...
    double
        lambda; // parameter to control the setp length in Marquardt levenberg optimization
...
static UTIL_SAFE_CAST_FUNCTION(
    rml_enkf_data,
    RML_ENKF_TYPE_ID) static UTIL_SAFE_CAST_FUNCTION_CONST(rml_enkf_data,
                                                           RML_ENKF_TYPE_ID)
...
static void rml_enkf_write_log_header(rml_enkf_data_type *data,
    ...
    if (rml_enkf_log_is_open(data->rml_log)) {
        ...
        rml_enkf_log_line(data->rml_log, format, column1, column2, column3,
                          column4, column5);
    }
}

static void rml_enkf_write_iter_info(rml_enkf_data_type *data, double prev_Sk,
                                     double Sk_new, double Std_new) {
    if (rml_enkf_log_is_open(data->rml_log)) {
        ...
        rml_enkf_log_line(data->rml_log, format, data->iteration_nr,
                          data->iteration_nr + 1, data->lambda, prev_Sk, Sk_new,
                          Std_new);
    }
}
...
static void rml_enkf_init1__(rml_enkf_data_type *data) {
    ...
    matrix_type *prior =
        matrix_alloc_column_compressed_copy(data->global_prior, data->ens_mask);
    ...
    matrix_type *Um =
        matrix_alloc(state_size, nrmin); /* Left singular vectors.  */
    matrix_type *VmT =
        matrix_alloc(nrmin, ens_size); /* Right singular vectors. */
    ...
    int nsign1 = enkf_linalg_svd_truncation(
        Dm, rml_enkf_config_get_truncation(data->config), -1, DGESVD_MIN_RETURN,
        Wm, Um, VmT);
...
static void rml_enkf_initA__(rml_enkf_data_type *data, matrix_type *A,
                             matrix_type *S, matrix_type *Cd, matrix_type *E,
                             matrix_type *D, matrix_type *Udr, double *Wdr,
                             matrix_type *VdTr) {
    ...
    // Perform SVD of tmp, where: tmp = diag_sqrt(Cd^(-1)) * centered(S) / sqrt(N-1) = Ud * Wd * Vd(T)
    {
        ...
        nsign = enkf_linalg_svd_truncation(
            tmp, rml_enkf_config_get_truncation(data->config), -1,
            DGESVD_MIN_RETURN, Wdr, Udr, VdTr);
        matrix_free(tmp);
    }
    ...
    // Calc X3
    {
        matrix_type *X3 = matrix_alloc(ens_size, ens_size);
        {
            ...
            enkf_linalg_rml_enkfX1(
                X1, Udr, D, Cd); // X1 = Ud(T)*Cd(-1/2)*D   -- D= -(dk-d0)
            enkf_linalg_rml_enkfX2(X2, Wdr, X1, data->lambda + 1,
                                   nsign); // X2 = ((a*Ipd)+Wd^2)^-1  * X1
        ...
        // Update A
        {
            ...
            matrix_subtract_row_mean(
                Dm); /* Remove the mean from the ensemble of model parameters*/
            matrix_scale(Dm, nsc);
...
void rml_enkf_init2__(rml_enkf_data_type *data, matrix_type *A, double *Wdr,
                      matrix_type *VdTr) {
...
static void rml_enkf_updateA_iter0(rml_enkf_data_type *data, matrix_type *A,
                                   matrix_type *S, matrix_type *R,
                                   matrix_type *dObs, matrix_type *E,
                                   matrix_type *D, matrix_type *Cd) {
...
void rml_enkf_updateA(void *module_data, matrix_type *A, matrix_type *S,
    ...
    matrix_type *Cd = matrix_alloc(
        nrobs, nrobs); // Cov(E), where E = measurement perturbations?

    // Empirical error covar. R is left unused. Investigate?
    enkf_linalg_Covariance(
        Cd, E, nsc, nrobs); // Cd = SampCov(E) (including (N-1) normalization)
    matrix_inv(Cd);         // In-place inversion

    rml_enkf_log_open(data->rml_log, data->iteration_nr);
    fprintf(stdout, "\nIter %d --> %d", data->iteration_nr,
            data->iteration_nr + 1);
    ...
    if (data->iteration_nr == 0) {
        ...
        matrix_type *Ud =
            matrix_alloc(nrobs, nrmin); // Left singular vectors.  */
        matrix_type *VdT =
            matrix_alloc(nrmin, ens_size); // Right singular vectors. */
        double *Wd = util_calloc(nrmin, sizeof *Wd); // Singular values, vector
        matrix_type *Skm = matrix_alloc(ens_size, ens_size); // Mismatch
        Sk_new = enkf_linalg_data_mismatch(
            D, Cd, Skm); // Skm = D'*inv(Cd)*D; Sk_new = trace(Skm)/N
        Std_new =
            matrix_diag_std(Skm, Sk_new); // Standard deviation of mismatches.
        ...
        */
                ...
                if (std_reduced)
                    data->lambda = data->lambda *
                                   rml_enkf_config_get_lambda_decrease_factor(
                                       data->config);

                rml_enkf_common_store_state(data->previous_state, A,
                                            data->ens_mask);
                ...
                data->lambda =
                    data->lambda *
                    rml_enkf_config_get_lambda_increase_factor(data->config);
                // A = data->previous_state
                rml_enkf_common_recover_state(data->previous_state, A,
                                              data->ens_mask);
            }
        }
...
void rml_enkf_init_update(void *arg, const bool_vector_type *ens_mask,
...
bool rml_enkf_set_double(void *arg, const char *var_name, double value) {
    rml_enkf_data_type *module_data = rml_enkf_data_safe_cast(arg);
    {
        ...
        if (strcmp(var_name, ENKF_TRUNCATION_KEY_) == 0)
            ...
            rml_enkf_config_set_lambda_increase_factor(module_data->config,
                                                       value);
        else if (strcmp(var_name, LAMBDA_REDUCE_FACTOR_KEY) == 0)
            rml_enkf_config_set_lambda_decrease_factor(module_data->config,
                                                       value);
...
double rml_enkf_get_double(const void *arg, const char *var_name) {
    const rml_enkf_data_type *module_data = rml_enkf_data_safe_cast_const(arg);
    {
        if (strcmp(var_name, LAMBDA_REDUCE_FACTOR_KEY) == 0)
            return rml_enkf_config_get_lambda_decrease_factor(
                module_data->config);

        if (strcmp(var_name, LAMBDA_INCREASE_FACTOR_KEY) == 0)
            return rml_enkf_config_get_lambda_increase_factor(
                module_data->config);
