...
struct rml_enkf_config_struct {
    ...
    long option_flags;
    bool
        use_prior; // Use exact/approximate scheme? Approximate scheme drops the "prior" term in the LM step.
...
rml_enkf_config_type *rml_enkf_config_alloc() {
    ...
    rml_enkf_config_set_option_flags(
    ...
    rml_enkf_config_set_lambda_decrease_factor(config,
                                               DEFAULT_LAMBDA_REDUCE_FACTOR);
    rml_enkf_config_set_lambda_increase_factor(config,
                                               DEFAULT_LAMBDA_INCREASE_FACTOR);
...
void rml_enkf_config_set_option_flags(rml_enkf_config_type *config,
                                      long flags) {
...
double
rml_enkf_config_get_lambda_increase_factor(rml_enkf_config_type *config) {
...
double
rml_enkf_config_get_lambda_decrease_factor(rml_enkf_config_type *config) {
...
bool rml_enkf_config_get_lambda_recalculate(
    const rml_enkf_config_type *config) {
...
void rml_enkf_config_free(rml_enkf_config_type *config) { free(config); }
