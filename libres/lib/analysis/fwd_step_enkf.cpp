...
static UTIL_SAFE_CAST_FUNCTION_CONST(
    fwd_step_enkf_data,
    FWD_STEP_ENKF_TYPE_ID) static UTIL_SAFE_CAST_FUNCTION(fwd_step_enkf_data,
                                                          FWD_STEP_ENKF_TYPE_ID)
...
void *fwd_step_enkf_data_alloc() {
    fwd_step_enkf_data_type *data =
        (fwd_step_enkf_data_type *)util_malloc(sizeof *data);
    ...
    data->option_flags =
        ANALYSIS_NEED_ED + ANALYSIS_UPDATE_A + ANALYSIS_SCALE_DATA;
...
static void
fwd_step_enkf_write_log_header(fwd_step_enkf_data_type *fwd_step_data,
                               const char *ministep_name, const int nx,
                               const int nd, const int ens_size) {
    ...
    if (fwd_step_log_is_open(fwd_step_data->fwd_step_log)) {
        ...
        fwd_step_log_line(fwd_step_data->fwd_step_log,
                          "CV folds                    : %d\n", nfolds);
        fwd_step_log_line(fwd_step_data->fwd_step_log,
                          "Relative R2 tolerance       : %f\n", r2_limit);
        fwd_step_log_line(
            fwd_step_data->fwd_step_log,
            "=================================================================="
            "=============================================================\n");
        fwd_step_log_line(fwd_step_data->fwd_step_log, format, column1, column2,
                          column3, column4);
        fwd_step_log_line(
            fwd_step_data->fwd_step_log,
            "=================================================================="
...
static void fwd_step_enkf_write_iter_info(fwd_step_enkf_data_type *data,
                                          stepwise_type *stepwise,
                                          const char *key,
                                          const int data_active_index,
    ...
    char *data_active_index_str =
        (char *)util_alloc_sprintf("(%d)", data_active_index);
    char *cat = util_strcat_realloc(loc_key, data_active_index_str);
    if (has_log)
        fwd_step_log_line(data->fwd_step_log, format, cat, global_index,
                          n_active);
    ...
    for (int ivar = 0; ivar < bool_vector_size(active_set); ivar++) {
        ...
        const module_obs_block_type *module_obs_block =
            module_obs_block_vector_search_module_obs_block(
                module_obs_block_vector, ivar);
        const int *active_indices =
            module_obs_block_get_active_indices(module_obs_block);
        bool all_active =
            active_indices == NULL; /* Inactive are not present in D */
        ...
        char *obs_list_entry = (char *)util_alloc_sprintf(
            format1, obs_key, obs_active_index, var_beta_percent);
...

    {
        ...
        for (int i = 0; i < stringlist_get_size(obs_list); i++) {
            const char *obs_list_entry =
                stringlist_iget(obs_list, perm_vector_iget(sort_perm, i));
            if (has_log)
...
/*Main function: */
void fwd_step_enkf_updateA(void *module_data, matrix_type *A,
                           const matrix_type *S0, const matrix_type *R,
                           const matrix_type *dObs, const matrix_type *E,
                           const matrix_type *D,
                           const module_info_type *module_info, rng_type *rng) {

    fwd_step_enkf_data_type *fwd_step_data =
        fwd_step_enkf_data_safe_cast(module_data);
...
#pragma omp master
        ...
        if (ens_size <= nfolds)
            util_abort("%s: The number of ensembles must be larger than the CV "
                       "fold - aborting\n",
                       __func__);

        {
            ...
            if (verbose) {
                char *ministep_name =
                    module_info_get_ministep_name(module_info);
                fwd_step_enkf_write_log_header(fwd_step_data, ministep_name, nx,
                                               nd, ens_size);
            }
            ...
            for (kw = 0; kw < num_kw; kw++) {
                module_data_block_type *data_block =
                    module_data_block_vector_iget_module_data_block(
                        data_block_vector, kw);
...
#pragma omp parallel for schedule(dynamic, 1)                                  \
    num_threads(fwd_step_data->num_threads)
            for (i = 0; i < nx; i++) {
                int kw_ind = int_vector_iget(kw_list, i);
                module_data_block_type *data_block =
                    module_data_block_vector_iget_module_data_block(
                        data_block_vector, kw_ind);
                const char *key = module_data_block_get_key(data_block);
                const int *active_indices =
                    module_data_block_get_active_indices(data_block);
                int active_index = 0;
                bool all_active =
                    active_indices == NULL; /* Inactive are not present in A */
                stepwise_type *stepwise_data =
                    stepwise_alloc1(ens_size, nd, rng, St, Et);
                ...
                if (verbose) {
                    ...
                    fwd_step_enkf_write_iter_info(fwd_step_data, stepwise_data,
                                                  key, active_index, i,
                                                  module_info);
                }
...
bool fwd_step_enkf_set_int(void *arg, const char *var_name, int value) {
    fwd_step_enkf_data_type *module_data = fwd_step_enkf_data_safe_cast(arg);
    {
        ...
        if (strcmp(var_name, NFOLDS_KEY) == 0)
            fwd_step_enkf_set_nfolds(module_data,
                                     value); /*Set number of CV folds */
        else if (strcmp(var_name, NUM_THREADS_KEY) == 0)
            fwd_step_enkf_set_num_threads(module_data,
                                          value); /*Set number of OMP threads */
        else
            name_recognized = false;
...
bool fwd_step_enkf_set_string(void *arg, const char *var_name,
                              const char *value) {
    fwd_step_enkf_data_type *module_data = fwd_step_enkf_data_safe_cast(arg);
    {
        bool name_recognized = true;
...
double fwd_step_enkf_get_double(const void *arg, const char *var_name) {
    const fwd_step_enkf_data_type *module_data =
        fwd_step_enkf_data_safe_cast_const(arg);
    {
        if (strcmp(var_name, R2_LIMIT_KEY) == 0)
