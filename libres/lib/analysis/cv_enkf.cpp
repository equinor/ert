...
static UTIL_SAFE_CAST_FUNCTION(
    cv_enkf_data,
    CV_ENKF_TYPE_ID) static UTIL_SAFE_CAST_FUNCTION_CONST(cv_enkf_data,
                                                          CV_ENKF_TYPE_ID)
...
void *cv_enkf_data_alloc() {
    ...
    data->option_flags =
        ANALYSIS_NEED_ED + ANALYSIS_USE_A + ANALYSIS_SCALE_DATA;
...
void cv_enkf_init_update(void *arg, const bool_vector_type *ens_mask,
    ...
    */
        ...
        matrix_type *V0T =
            matrix_alloc(nrmin, nrens); /* Right singular vectors. */
        ...
        enkf_linalg_svdS(S, cv_data->truncation, cv_data->subspace_dimension,
                         DGESVD_MIN_RETURN, inv_sig0, U0, V0T);
    ...
    */
        ...
        /* Also compute Rp */
        {
            ...
            matrix_dgemm(cv_data->Rp, X0, U0, false, false, 1.0,
                         0.0); /* Rp = X0 * U0 */
            matrix_free(X0);
        }
...
static void cv_enkf_get_cv_error_prin_comp(
    cv_enkf_data_type *cv_data, matrix_type *cvErr, const matrix_type *A,
    const int *indexTest, const int *indexTrain, const int nTest,
    const int nTrain, const int foldIndex, const int maxP) {
    /*
  ...
  */
    ...
    for (p = 0; p < maxP; p++) {
        ...
        for (i = 0; i <= p; i++)
            for (j = 0; j < nTrain; j++)
                matrix_iset(ZpTrain, i, j,
                            matrix_iget(cv_data->Z, i, indexTrain[j]));
        ...
        for (i = 0; i <= p; i++)
            for (j = 0; j <= p; j++)
                matrix_iadd(SigDp, i, j,
                            (nTrain - 1) * matrix_iget(cv_data->Rp, i, j));

        /* Invert the covariance matrix for the principal components  */
        {
            int inv_ok = matrix_inv(SigDp);
            if (inv_ok != 0)
                util_abort("%s: inversion of covariance matrix for the "
                           "principal components failed for subspace dimension "
                           "p = %d\n - aborting \n",
                           __func__, p + 1);
        }

        {
            ...
            for (i = 0; i <= p; i++) {
                for (j = 0; j < nTest; j++) {
                    double ksum = 0.0;
                    for (int k = 0; k <= p; k++)
                        ksum += matrix_iget(SigDp, i, k) *
                                matrix_iget(cv_data->Z, k, indexTest[j]);
        ...
        /*Compute Press Statistic: */
        {
            ...
            for (i = 0; i < nx; i++) {
                for (j = 0; j < nTest; j++) {
                    double tmp = matrix_iget(A, i, indexTest[j]) -
                                 matrix_iget(AHat, i, j);
                    R2Sum += tmp * tmp;
                }
            }
...
int cv_enkf_get_optimal_numb_comp(cv_enkf_data_type *cv_data,
                                  const matrix_type *cvErr, const int maxP) {
...
static int get_optimal_principal_components(cv_enkf_data_type *cv_data,
    ...
    if (maxP > nrmin)
        maxP =
            nrmin - 1; // <- Change by Joakim; using nrmin here will load to out
                       // bounds access oc cv_data->Z at line 460.

    if (nrens < cv_data->nfolds)
        util_abort("%s: number of ensemble members %d need to be larger than "
                   "the number of cv-folds - aborting \n",
                   __func__, nrens, cv_data->nfolds);
    ...
    cvError = matrix_alloc(maxP, cv_data->nfolds);
    {
        ...
        for (i = 0; i < cv_data->nfolds; i++) {
            ...
            cv_enkf_get_cv_error_prin_comp(cv_data, cvError, A, indexTest,
                                           indexTrain, ntest, ntrain, i, maxP);
        }
        free(indexTest);
        free(indexTrain);
    }
...
static void getW_prin_comp(cv_enkf_data_type *cv_data, matrix_type *W,
                           const int optP) {
    ...
    for (i = 0; i < optP; i++) {
        for (j = 0; j < optP; j++) {
            tmp2 = matrix_iget(SigZp, i, j) +
                   (nrens - 1) * matrix_iget(cv_data->Rp, i, j);
    ...
    if (inv_ok != 0)
        util_abort(
            "%s: inversion of covariance matrix for the principal components "
            "failed for subspace dimension p = %d\n - aborting \n",
            __func__, optP);
            ...
            for (int i = 0; i < ens_size; i++) {
                for (int j = 0; j < ens_size; j++) {
                    ...
                    for (int k = 0; k < optP; k++)
                        tmp += matrix_iget(W, i, k) *
                               matrix_iget(cv_data->Dp, k, j);
