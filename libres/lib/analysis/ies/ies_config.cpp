...
ies::config_type *ies::config_alloc() {
    ies::config_type *config =
        static_cast<ies::config_type *>(util_malloc(sizeof *config));
    UTIL_TYPE_ID_INIT(config, IES_CONFIG_TYPE_ID);
    config->ies_logfile = NULL;
    ies::config_set_truncation(config, DEFAULT_TRUNCATION);
    ies::config_set_subspace_dimension(config, DEFAULT_SUBSPACE_DIMENSION);
    ies::config_set_option_flags(config, ANALYSIS_NEED_ED + ANALYSIS_UPDATE_A +
                                             ANALYSIS_ITERABLE +
                                             ANALYSIS_SCALE_DATA);
    ies::config_set_max_steplength(config, DEFAULT_IES_MAX_STEPLENGTH);
    ies::config_set_min_steplength(config, DEFAULT_IES_MIN_STEPLENGTH);
    ies::config_set_dec_steplength(config, DEFAULT_IES_DEC_STEPLENGTH);
    ies::config_set_subspace(config, DEFAULT_IES_SUBSPACE);
    ies::config_set_inversion(config, DEFAULT_IES_INVERSION);
    ies::config_set_logfile(config, DEFAULT_IES_LOGFILE);
    ies::config_set_aaprojection(config, DEFAULT_IES_AAPROJECTION);
...
double ies::config_get_truncation(const config_type *config) {
    return config->truncation;
}

void ies::config_set_truncation(config_type *config, double truncation) {
...
int ies::config_get_subspace_dimension(const ies::config_type *config) {
    return config->subspace_dimension;
}

void ies::config_set_subspace_dimension(config_type *config,
...
long ies::config_get_option_flags(const ies::config_type *config) {
    return config->option_flags;
}

void ies::config_set_option_flags(config_type *config, long flags) {
    config->option_flags = flags;
}

/*------------------------------------------------------------------------------------------------*/
/* IES_MAX_STEPLENGTH */
double ies::config_get_max_steplength(const ies::config_type *config) {
    return config->ies_max_steplength;
}
void ies::config_set_max_steplength(ies::config_type *config,
                                    double ies_max_steplength) {
    config->ies_max_steplength = ies_max_steplength;
}
/*------------------------------------------------------------------------------------------------*/
/* IES_MIN_STEPLENGTH */
double ies::config_get_min_steplength(const ies::config_type *config) {
    return config->ies_min_steplength;
}
void ies::config_set_min_steplength(ies::config_type *config,
...
/*------------------------------------------------------------------------------------------------*/
/* IES_DEC_STEPLENGTH */
double ies::config_get_dec_steplength(const ies::config_type *config) {
    return config->ies_dec_steplength;
}
void ies::config_set_dec_steplength(ies::config_type *config,
...
/*------------------------------------------------------------------------------------------------*/
/* IES_INVERSION          */
ies::inversion_type ies::config_get_inversion(const ies::config_type *config) {
    return static_cast<ies::inversion_type>(config->ies_inversion);
}
void ies::config_set_inversion(ies::config_type *config,
...
/*------------------------------------------------------------------------------------------------*/
/* IES_SUBSPACE      */
bool ies::config_get_subspace(const ies::config_type *config) {
    return config->ies_subspace;
}
void ies::config_set_subspace(ies::config_type *config, bool ies_subspace) {
    config->ies_subspace = ies_subspace;
}

/*------------------------------------------------------------------------------------------------*/
/* IES_AAPROJECTION         */
bool ies::config_get_aaprojection(const ies::config_type *config) {
    return config->ies_aaprojection;
}
void ies::config_set_aaprojection(config_type *config, bool ies_aaprojection) {
    config->ies_aaprojection = ies_aaprojection;
}

/*------------------------------------------------------------------------------------------------*/
/* IES_LOGFILE       */
char *ies::config_get_logfile(const ies::config_type *config) {
    return config->ies_logfile;
}
void ies::config_set_logfile(ies::config_type *config,
...
/*------------------------------------------------------------------------------------------------*/
/* FREE_CONFIG */
void ies::config_free(ies::config_type *config) { free(config); }

double ies::config_calculate_steplength(const ies::config_type *ies_config,
                                        int iteration_nr) {
    double ies_max_step = ies::config_get_max_steplength(ies_config);
    double ies_min_step = ies::config_get_min_steplength(ies_config);
    double ies_decline_step = ies::config_get_dec_steplength(ies_config);
