void static teccost(const matrix_type *W, const matrix_type *D,
                    const char *fname, const int ens_size, const int izone) {
    FILE *fp;
...
void static teclog(const matrix_type *W, const matrix_type *D,
                   const matrix_type *DW, const char *fname, const int ens_size,
                   const int itnr, const double rcond, const int nrsing,
                   const int nrobs) {
    ...
    if (itnr == 1) {
        fp = fopen(fname, "w");
        fprintf(fp, "TITLE = \"%s\"\n", fname);
        fprintf(fp, "VARIABLES = \"it\" \"Diff1W\" \"Diff2W\" \"J-prior\" "
                    "\"J-data\" \"J\" \"rcond\" \"Sing\" \"nrobs\"\n");
    } else {
        fp = fopen(fname, "a");
    }
    ...
    fprintf(fp, " %d %12.5e %12.5e %12.5e %12.5e %12.5e %12.5e %d %d\n",
            itnr - 1, diff1W, diff2W, costfp, costfd, costf, rcond, nrsing,
            nrobs);
    fclose(fp);
}
