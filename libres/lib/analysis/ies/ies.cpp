...
void ies_initX__(const std::optional<Eigen::MatrixXd> &A, const Eigen::MatrixXd &Y0,
                 const Eigen::MatrixXd &R, const Eigen::MatrixXd &E,
                 const Eigen::MatrixXd &D, Eigen::MatrixXd &X,
                 const ies::config::inversion_type ies_inversion,
                 const std::variant<double, int> &truncation,
                 bool use_aa_projection, Eigen::MatrixXd &W0, double ies_steplength,
...

{
    ...
    if (A) {
        const int state_size = A.value().rows();
        if (use_aa_projection && (state_size <= (ens_size - 1))) {
            ies::linalg_compute_AA_projection(A.value(), Y);
        }
    }
...
void ies::updateA(
    ...
    const Eigen::MatrixXd &Rin,   // Measurement error covariance matrix (not used)
    const Eigen::MatrixXd &Ein,   // Ensemble of observation perturbations
    const Eigen::MatrixXd &Din) { // (d+E-Y) Ensemble of perturbed observations - Y

    int ens_size = Yin.cols(); // Number of active realizations in current iteration
    ...
    */
     ...
     */
    Eigen::MatrixXd Y = Yin;
    Eigen::MatrixXd R = Rin;
...
/*  COMPUTING THE PROJECTION Y= Y * (Ai^+ * Ai) (only used when state_size < ens_size-1)    */
void ies::linalg_compute_AA_projection(const Eigen::MatrixXd &A, Eigen::MatrixXd &Y) {
    int ens_size = A.cols();
    int state_size = A.rows();
    int nrobs = Y.rows();
...
void ies::linalg_solve_S(const Eigen::MatrixXd &W0, const Eigen::MatrixXd &Y,
                         Eigen::MatrixXd &S) {
    int ens_size = W0.cols();
    int nrobs = S.rows();
    double nsc = 1.0 / sqrt(ens_size - 1.0);

    Eigen::MatrixXd YT =
        Eigen::MatrixXd::Zero(ens_size, nrobs); // Y^T used in linear solver
    Eigen::MatrixXd ST =
        Eigen::MatrixXd::Zero(ens_size, nrobs); // current S^T used in linear solver

    /*  Here we compute the W (I-11'/N) / sqrt(N-1)  and transpose it).*/
    // Omega=data->W (from previous iteration used to solve for S)
    Eigen::MatrixXd Omega = W0; // current S^T used in linear solver
    ...
    YT = Y.transpose(); // RHS stored in YT
...
*          S'*(S*S'+R)^{-1} H           (a)
*/
void ies::linalg_subspace_inversion(Eigen::MatrixXd &W0, const int ies_inversion,
                                    const Eigen::MatrixXd &E, const Eigen::MatrixXd &R,
...
*/
void ies::linalg_store_active_W(ies::data::Data *data, const Eigen::MatrixXd &W0) {
...
void ies::initX(const config::Config &ies_config, const Eigen::MatrixXd &Y0,
    ...
    Eigen::MatrixXd W0 = Eigen::MatrixXd::Zero(active_ens_size, active_ens_size);
