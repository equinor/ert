...
int main(int argc, const char **argv) {
    ...
    enkf_main_install_SIGNALS();
    {
        ert_test_context_type *test_context =
            ert_test_context_alloc("std_scale_test", config_file);
        stringlist_type *args = stringlist_alloc_new();

        ert_test_context_install_workflow_job(test_context, "STD_SCALE",
                                              workflow_job_file);
        stringlist_append_copy(args, "WWCT:OP_1");
        test_assert_true(
            ert_test_context_run_worklow_job(test_context, "STD_SCALE", args));
