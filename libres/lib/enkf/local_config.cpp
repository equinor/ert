...
LocalConfig::LocalConfig(const std::vector<std::string>& parameter_keys, const std::vector<std::string>& obs_keys)
    : m_global_updatestep("DEFAULT")
    , m_global_ministep("ALL_ACTIVE")   // The strings "ALL_ACTIVE" and "ALL_OBS" are part of the API.
...
LocalObsData * LocalConfig::global_obsdata() {
    return &this->m_global_obsdata;
}
...
void LocalConfig::clear() const {
    logger->warning("The LocalConfig::clear() function is deprecated and will be removed");
}
...
RES_LIB_SUBMODULE("local.local_config", m) {
    py::class_<LocalConfig>(m, "LocalConfig")
        .def("createMinistep", &LocalConfig::make_ministep, py::return_value_policy::reference_internal)
        .def("createObsdata", &LocalConfig::make_obsdata, py::return_value_policy::reference_internal)
        .def("getUpdatestep", &LocalConfig::updatestep, py::return_value_policy::reference_internal)
