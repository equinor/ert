...
local_dataset_type *local_dataset_alloc_copy(local_dataset_type *src_dataset,
    ...
    while (!hash_iter_is_complete(size_iter)) {
        ...
        active_list_type *active_list = active_list_alloc_copy(
            (const active_list_type *)hash_get(src_dataset->active_size, key));
        hash_insert_hash_owned_ref(copy_dataset->active_size, key, active_list,
                                   active_list_free__);
    }
...
void local_dataset_add_node(local_dataset_type *dataset, const char *node_key) {
    ...
    hash_insert_hash_owned_ref(dataset->active_size, node_key,
                               active_list_alloc(), active_list_free__);
}
                                   ...
                                   FILE *stream) {
    {
        ...
        while (!hash_iter_is_complete(data_iter)) {
            ...
            active_list_type *active_list =
                (active_list_type *)hash_get(dataset->active_size, data_key);
            active_list_summary_fprintf(
