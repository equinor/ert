...
C_USED void *enkf_main_exit_JOB(void *self, const stringlist_type *args) {
...
*/
// Internal workflow job
C_USED void *enkf_main_select_case_JOB(void *self,
...
// Internal workflow job
C_USED void *enkf_main_create_case_JOB(void *self,
                                       const stringlist_type *args) {
    enkf_main_type *enkf_main = enkf_main_safe_cast(self);
    const char *new_case = stringlist_iget(args, 0);
...
// Internal workflow job
C_USED void *enkf_main_load_results_JOB(void *self,
                                        const stringlist_type *args) {
    enkf_main_type *enkf_main = enkf_main_safe_cast(self);
    int iter = 0;
    {
...
// Internal Workflow job
C_USED void *enkf_main_load_results_iter_JOB(void *self,
                                             const stringlist_type *args) {
    enkf_main_type *enkf_main = enkf_main_safe_cast(self);
...
// Internal workflow job
C_USED void *enkf_main_export_field_JOB(void *self,
                                        const stringlist_type *args) {
    const char *file_name = stringlist_iget(args, 1);
    field_file_format_type file_type =
...
// Internal workflow job
C_USED void *enkf_main_export_field_to_RMS_JOB(void *self,
                                               const stringlist_type *args) {
    enkf_main_type *enkf_main = enkf_main_safe_cast(self);
...
// Internal workflow job
C_USED void *enkf_main_export_field_to_ECL_JOB(void *self,
                                               const stringlist_type *args) {
    enkf_main_type *enkf_main = enkf_main_safe_cast(self);
...
// Internal workflow job
C_USED void *enkf_main_rank_on_observations_JOB(void *self,
                                                const stringlist_type *args) {
    enkf_main_type *enkf_main = enkf_main_safe_cast(self);
    ...
    int delimiter = 0;
    {
        ...
        if (delimiter > -1) {
            ...
            obs_arguments =
                (stringlist_get_size(args) > delimiter + 1) ? true : false;
    ...
    int_vector_type *steps_vector = NULL;
    {
        ...
        if (step_arguments)
            report_steps =
                stringlist_alloc_joined_substring(args, 1, delimiter, ",");
        else
            report_steps = util_alloc_sprintf(
                "0-%d", enkf_main_get_history_length(enkf_main));
    ...
    stringlist_type *obs_ranking_keys = NULL;
    {
        ...
        if (obs_arguments)
            obs_key_char = stringlist_alloc_joined_substring(
                args, delimiter + 1, stringlist_get_size(args), " ");

        enkf_obs_type *enkf_obs = enkf_main_get_obs(enkf_main);
        obs_ranking_keys =
            enkf_obs_alloc_matching_keylist(enkf_obs, obs_key_char);

        if ((obs_arguments) && (stringlist_get_size(obs_ranking_keys) == 0)) {
            fprintf(stderr,
                    "The input string : \"%s\" did not resolve to any valid "
                    "observation keys. Job not started\n",
                    obs_key_char);
            return NULL;
        }
    ...
    enkf_main_rank_on_observations(enkf_main, ranking_name, obs_ranking_keys,
                                   steps_vector);
...
// Internal workflow job
C_USED void *enkf_main_export_runpath_file_JOB(void *self,
                                               const stringlist_type *args) {
    enkf_main_type *enkf_main = enkf_main_safe_cast(self);
...
// Internal workflow job
C_USED void *
enkf_main_std_scale_correlated_obs_JOB(void *self,
                                       const stringlist_type *args) {
    printf("The STD_SCALE_CORRELATED_OBS workflow is deprecated. "
           "Use the 'auto_scale' option in the MISFIT_PREPROCESSOR workflow "
           "instead.\n");
    ...
    if (stringlist_get_size(args) > 0) {
        ...
        for (int iarg = 0; iarg < stringlist_get_size(args); iarg++) {
            ...
            stringlist_type *key_list =
                enkf_obs_alloc_matching_keylist(obs, arg_key);
            if (verbose)
                printf("%s => ", arg_key);
            ...
            for (int iobs = 0; iobs < stringlist_get_size(key_list); iobs++) {
                ...
                const obs_vector_type *obs_vector =
                    enkf_obs_get_vector(obs, obs_key);
                local_obsdata_add_node(obsdata,
                                       obs_vector_alloc_local_node(obs_vector));
                if (verbose)
                    printf("%s ", obs_key);
            }
            if (verbose && (stringlist_get_size(key_list) == 0))
                printf("**ERROR - no observation keys matched argument %s - "
                       "ignored\n",
                       arg_key);
        ...
        if (local_obsdata_get_size(obsdata) > 0) {
            ...
            enkf_obs_scale_correlated_std(obs, fs, realizations, obsdata, alpha,
                                          std_cutoff, verbose);
        } else if (verbose)
            printf("**Warning: Your list of arguments did not match any "
                   "observation keys - no scaling performed.\n");

        int_vector_free(realizations);
        local_obsdata_free(obsdata);
    } else
        fprintf(stderr, "** Warning: When using the workflow job to scale "
                        "correlated observations you must provide the "
                        "observation keys as arguments\n");

    return NULL;
}

// Internal workflow job
C_USED void *enkf_main_analysis_update_JOB(void *self,
                                           const stringlist_type *args) {
    enkf_main_type *enkf_main = enkf_main_safe_cast(self);
    enkf_fs_type *source_fs =
...
// Internal workflow job
C_USED void *enkf_main_pre_simulation_copy_JOB(void *self,
                                               const stringlist_type *args) {
