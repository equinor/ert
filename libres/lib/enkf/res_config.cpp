...
static void res_config_install_config_key(config_parser_type *config_parser,
                                          config_content_type *config_content,
                                          const char *key, const char *value) {

    config_schema_item_type *schema_item =
        config_get_schema_item(config_parser, key);
    ...
    config_content_item_type *content_item =
        config_content_get_item(config_content, key);

    config_content_node_type *new_node =
        config_content_item_alloc_node(content_item, NULL);
    config_content_node_add_value(new_node, value);

    if (!new_node)
        util_abort("%s: Failed to internally install %s: %s\n", __func__, key,
                   value);
...
res_config_alloc_user_content(const char *user_config_file,
    ...
    char *res_config_file =
        (user_config_file ? util_alloc_realpath(user_config_file) : NULL);

    res_config_install_config_key(config_parser, config_content,
                                  RES_CONFIG_FILE_KEY, res_config_file);

    // Install working directory
    char *res_config_dir = res_config_alloc_config_directory(res_config_file);

    res_config_install_config_key(config_parser, config_content,
                                  CONFIG_DIRECTORY_KEY, res_config_dir);
...
res_config_get_analysis_config(const res_config_type *res_config) {
    return res_config->analysis_config;
}

ert_workflow_list_type *
res_config_get_workflow_list(const res_config_type *res_config) {
