...
static void data_ranking_init(data_ranking_type *ranking, enkf_fs_type *fs,
                              const enkf_config_node_type *config_node,
                              const char *key_index, int step) {
    ...
    if (ranking->sort_increasing)
        ranking->sort_permutation =
            double_vector_alloc_sort_perm(ranking->data_ensemble);
    else
        ranking->sort_permutation =
            double_vector_alloc_rsort_perm(ranking->data_ensemble);
...
data_ranking_get_permutation(const data_ranking_type *data_ranking) {
        ...
        for (i = 0; i < ens_size; i++) {
            ...
            if (bool_vector_iget(data_ranking->valid, iens))
                fprintf(stream, "%3d    %3d          %14.3f\n", i, iens,
                        double_vector_iget(data_ranking->data_ensemble, iens));
        }
        fprintf(stream, "----------------------------------\n");
    }
}
