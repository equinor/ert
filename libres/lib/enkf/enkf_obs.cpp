        ...
        if (local_obsdata_node_tstep_active(obs_node, step) &&
            ...
            const summary_obs_type *summary_obs =
                (const summary_obs_type *)obs_vector_iget_node(obs_vector,
                                                               step);
            double_vector_iset(obs_std, active_count,
                               summary_obs_get_std(summary_obs) *
                                   summary_obs_get_std_scaling(summary_obs));
            double_vector_iset(obs_value, active_count,
                               summary_obs_get_value(summary_obs));
            last_step = step;
            active_count++;
        }
    }
        ...
        while (true) {
            ...
            if (local_obsdata_node_tstep_active(obs_node, step) &&
                ...
                for (int iens_index = 0; iens_index < active_size;
                     iens_index++) {
                    const int iens =
                        int_vector_iget(ens_active_list, iens_index);
                    ...
                    if (step >= smlength) {
                        ...
                        obs_block_deactivate(obs_block, active_count, true,
                                             msg);
                        free(msg);
                        break;
                    } else {
                        meas_block_iset(
                            ...
                            summary_get((const summary_type *)
                                            enkf_node_value_ptr(work_node),
                                        node_id.report_step));
                    }
                }
                active_count++;
...
void enkf_obs_get_obs_and_measure_node(const enkf_obs_type *enkf_obs,
    ...
    if (obs_type == SUMMARY_OBS) {
        ...
        enkf_obs_get_obs_and_measure_summary(enkf_obs, obs_vector, fs, obs_node,
                                             ens_active_list, meas_data,
                                             obs_data, work_value, work_std);
...
double enkf_obs_scale_correlated_std(const enkf_obs_type *enkf_obs,
    ...
    enkf_obs_get_obs_and_measure_data(enkf_obs, fs, local_obsdata,
                                      ens_active_list, meas_data, obs_data);
    ...
    if (obs_data_get_active_size(obs_data) > 1) {
        ...
        if (S) {
            ...
            if (verbose) {
                ...
                for (int i = 0; i < local_obsdata_get_size(local_obsdata);
                     i++) {
                    const local_obsdata_node_type *node =
                        local_obsdata_iget(local_obsdata, i);
