...
enkf_plot_gen_kw_iget(const enkf_plot_gen_kw_type *plot_gen_kw, int iens) {
    if ((iens < 0) || (iens >= plot_gen_kw->size))
        util_abort("%s: index:%d invalid. Valid interval: [0,%d>.\n", __func__,
                   iens, plot_gen_kw->size);

    return plot_gen_kw->ensemble[iens];
}

static void enkf_plot_gen_kw_resize(enkf_plot_gen_kw_type *plot_gen_kw,
                                    int new_size) {
    if (new_size != plot_gen_kw->size) {
        ...
        plot_gen_kw->ensemble = (enkf_plot_gen_kw_vector_type **)util_realloc(
            plot_gen_kw->ensemble, new_size * sizeof *plot_gen_kw->ensemble);

        if (new_size > plot_gen_kw->size) {
            for (iens = plot_gen_kw->size; iens < new_size; iens++) {
                plot_gen_kw->ensemble[iens] = enkf_plot_gen_kw_vector_alloc(
                    plot_gen_kw->config_node, iens);
            }
        }
...
void enkf_plot_gen_kw_load(enkf_plot_gen_kw_type *plot_gen_kw, enkf_fs_type *fs,
        ...
        for (iens = 0; iens < ens_size; ++iens) {
            if (bool_vector_iget(mask, iens)) {
                enkf_plot_gen_kw_vector_type *vector =
                    enkf_plot_gen_kw_iget(plot_gen_kw, iens);
                enkf_plot_gen_kw_vector_load(vector, fs, transform_data,
                                             report_step);
            }
        }
    }
