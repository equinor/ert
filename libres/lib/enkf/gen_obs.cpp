...
void gen_obs_measure(const gen_obs_type *gen_obs, const gen_data_type *gen_data,
    ...
    gen_obs_assert_data_size(gen_obs, gen_data);
    {
        int active_size = __active_list->getActiveSize(gen_obs->obs_size);
...
C_USED void gen_obs_update_std_scale(gen_obs_type *gen_obs,
    ...
    if (active_list->getMode() == ALL_ACTIVE) {
        ...
        int size = active_list->getActiveSize(gen_obs->obs_size);
...
void update_std_scaling(py::handle obj, double scaling,
                        const ActiveList &active_list) {
    auto *gen_obs = reinterpret_cast<gen_obs_type *>(
        PyLong_AsVoidPtr(obj.attr("_BaseCClass__c_pointer").ptr()));
    gen_obs_update_std_scale(gen_obs, scaling, &active_list);
}
} // namespace
