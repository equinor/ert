...
TEST_CASE("ies_enkf_linalg_extract_active_E", "[analysis]") {
    ...
    ies::enkf_store_initial_obs_mask(data, obs_mask);
...
TEST_CASE("ies_enkf_linalg_extract_active_W", "[analysis]") {
    ...
    ies::enkf_config_set_logfile(ies::enkf_data_get_config(data), "log");
    ies::enkf_data_open_log(data);
    ies::enkf_init_update(data, ens_mask, obs_mask, nullptr, nullptr, nullptr,
    ...
    {
        matrix_type *W = ies::enkf_alloc_activeW(data);
...
SCENARIO("ies_enkf_linalg_extract_active_A", "[analysis]") {
    GIVEN("Inital setup") {
        ...
        ies::enkf_init_update(data, ens_mask, obs_mask, nullptr, nullptr,
                              nullptr, nullptr, nullptr, nullptr);
        ies::enkf_data_store_initialA(data, A0);

        WHEN("All realizations active") {
            matrix_type *A = ies::enkf_alloc_activeA(data);
