...
class LocalDatasetTest(ResTest):
    def setUp(self):
        ...
        self.surf_config = self.createTestPath(
            "local/snake_oil_field/snake_oil_surface.ert"
        )
    ...
    def test_local_field(self):
        with ErtTestContext(
            "python/enkf/data/local_config", self.field_config
        ) as test_context:
    ...
    def test_local_surface(self):
        with ErtTestContext(
            "python/enkf/data/local_config", self.surf_config
        ) as test_context:
            main = test_context.getErt()
            local_config = main.getLocalConfig()
