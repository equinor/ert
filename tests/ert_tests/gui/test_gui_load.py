...
from src.gui.widgets.message_box import ErtMessageBox
from src.gui.main import _start_window, run_gui
...
def patch_enkf_main(monkeypatch, tmpdir):
    ...
    monkeypatch.setattr(
        src.gui.simulation.simulation_panel,
        "LibresFacade",
        Mock(return_value=facade_mock),
    )

    monkeypatch.setattr(
        src.gui.simulation.ensemble_smoother_panel,
        "LibresFacade",
        Mock(return_value=facade_mock),
    )
    monkeypatch.setattr(src.gui.main, "EnKFMain", Mock(return_value=mocked_enkf_main))
    monkeypatch.setattr(src.gui.main, "ResConfig", Mock(return_value=res_config_mock))
    monkeypatch.setattr(
        src.gui.widgets.caseselector.CaseSelector,
        "_getAllCases",
    ...
    monkeypatch.setattr(
        src.gui.widgets.summarypanel.ErtSummary,
        "getForwardModels",
        Mock(return_value=[]),
    )
    monkeypatch.setattr(
...
def test_gui_full(monkeypatch, tmpdir, qapp, mock_start_server):
    with tmpdir.as_cwd():
        ...
        monkeypatch.setattr(src.gui.main, "QApplication", Mock(return_value=qapp))
        run_gui(args_mock)
...
def test_gui_iter_num(monkeypatch, tmpdir, qtbot, patch_enkf_main):
    ...
    monkeypatch.setattr(
        src.gui.simulation.simulation_panel.SimulationPanel,
        "runSimulation",
