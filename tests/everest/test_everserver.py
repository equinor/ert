...
from ert.dark_storage.endpoints.experiment_runner import (
...
def test_websocket_no_authentication(setup_client):
    ...
    with (
        client.websocket_connect(
            f"/experiment_runner/events/{experiment_id}"
        ) as websocket,
...
def test_that_multiple_started_experiments_each_receive_distinct_experiment_ids(
    ...
    try:
        ...
        with (
            ...
            patch(
                "ert.dark_storage.endpoints.experiment_runner.ExperimentRunner",
                return_value=mock_runner,
            ),
        ):
            r1 = client.post(
                "/experiment_runner/start_experiment",
        ...
        runs_response = client.get(
            "/experiment_runner/experiments", headers=auth_headers
        )
...
def test_that_get_status_by_experiment_id_endpoint_returns_expected_response(
    ...
    response = client.get(
        f"/experiment_runner/status/{experiment_id or valid_experiment_id}",
        headers={"Authorization": f"Basic {credentials}"},
    )
...
def test_that_get_config_path_returns_not_found_for_pending_experiment_state(
    ...
    response = client.get(
        f"/experiment_runner/config_path/{experiment_id}",
        headers={"Authorization": f"Basic {credentials}"},
    )
