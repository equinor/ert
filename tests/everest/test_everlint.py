...
def test_commented_out_substitution(min_config, change_to_tmpdir):
    ...
    with fileinput.input("config.yml", inplace=True) as fin:
        for _, line in enumerate(fin):
            if "job2 -c config.yml" in line:
                print(" # - job2 -c r{{sub}} config.yml")
            else:
                print(line, end="")
    ...
    assert config_dict["forward_model"][0] == "job1 -c config.yml"
    assert config_dict["forward_model"][1] == "job3 -c config.yml"
