...
@patch("everest.config.ServerConfig.get_server_context_from_client")
@patch(
    "ert.services.StorageService.session",
    side_effect=[TimeoutError("Boom!"), MagicMock()],
)
@patch(
...
def test_everest_entry_debug(
    ...
    session_mock,
    get_server_context_from_client_mock,
    start_server_mock,
    ...
    assert get_server_context_from_client_mock.call_count == 2
...
@patch("ert.services.StorageService.session", side_effect=TimeoutError("Boom!"))
@patch("everest.config.ServerConfig.get_server_context_from_client")
def test_everest_entry_detached_already_run_monitor(
    ...
    everserver_status_mock.assert_called()
    everserver_status_mock.reset_mock()
    get_server_context_from_client_mock.assert_not_called()
...
def test_everest_entry_detached_running(
    ...
    session_mock.assert_called_once()
    session_mock.reset_mock()
    get_server_context_from_client_mock.assert_called_once()
    wait_for_server_mock.assert_not_called()
    everserver_status_mock.assert_not_called()
