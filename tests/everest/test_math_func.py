...
def test_math_func_minimal():
    ...
    # Check the optimum value
    assert workflow.result.total_objective == pytest.approx(0.0, abs=0.005)

    # Test conversion to pandas DataFrame
    df = export(config)

    # find first and last sims that succeeded and not a gradient
...
def test_math_func_out_constr():
    ...
    # All points in this case are increasing the merit
