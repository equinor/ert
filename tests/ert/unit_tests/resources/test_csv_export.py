...
def test_that_csv_export_matches_snapshot(monkeypatch, tmp_path, snapshot):
    ...
    with open_storage(tmp_path / "storage", mode="w") as storage:
        ...
        ens.save_parameters(
            group=info.gen_kw_config.name,
            realization=None,
            dataset=info.genkw_data,
        )
        ...
        df = pl.read_csv(output_file).select(pl.col(pl.Float64).round(2))

        snapshot.assert_match(df.write_csv(), "csv_export_result.csv")
