...
def test_that_migration_will_not_delete_files_outside_blob_directory(
    tmp_path,
):
    root = tmp_path / "project"
    ...
    outside_blob = ens_dir / "outside.blob"
    outside_blob.write_bytes(b"outside")
    (blob_dir / "malicious.blob.json").write_text(
        json.dumps(
            {
                "uri": "../outside.blob",
                "file_size": 7,
    ...
    assert (blob_dir / "malicious.blob.json").exists()
