...
from ert.gui.experiments.view import UpdatesWidget
...
def test_that_added_update_is_retrievable_by_iteration(qtbot):
    ...
    first = widget.add_update(0)
    second = widget.add_update(1)

    assert widget.update_widget(0) is first
    assert widget.update_widget(1) is second
