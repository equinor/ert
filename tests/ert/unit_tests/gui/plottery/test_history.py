...
from ert.gui.tools.plot.plottery.plots.history import plotHistory
...
def test_that_history_plot_returns_early_if_history_is_disabled_or_lacking_data(
    ...
    plotHistory(generic_plot_context, axes)
    axes_lines = axes.get_lines()
    assert len(axes_lines) == 0


def test_that_plotHistory_returns_lines_if_history_is_enabled(
    plot_context_with_history, axes
):
    plotHistory(plot_context_with_history, axes)
