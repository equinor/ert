...
def test_that_active_realizations_selector_validates_with_ensemble_size_from_prior(
    ...
    prior_ensemble0_number_of_realizations_with_responses = 6
    prior_ensemble0_ensemble_size = 8

    prior_ensemble1_number_of_realizations_with_responses = 3
    ...
    assert (
        ...
        == f"<b>{prior_ensemble0_number_of_realizations_with_responses}</b>"
    )
    ...
    assert (
        panel._number_of_realizations_label.text()
        == f"<b>{prior_ensemble1_number_of_realizations_with_responses}</b>"
    )
