...
def test_report_with_successful_start_message_argument(unused_tcp_port):
    ...
    lines = []
    with mock_zmq_thread(host, unused_tcp_port, lines):
...
def test_report_with_failed_reporter_but_finished_jobs(unused_tcp_port):
    ...
    with mock_zmq_thread(host, unused_tcp_port, lines):
        ...
        assert reporter._done
    assert len(lines) == 0, "expected 0 Job running messages"

    # TODO refactor or remove, zmq handles reconnection automatically
    # @pytest.mark.integration_test
    ...
    #             _wait_until(
    #                 condition=lambda: patched_send.call_count >= 1,
    #                 timeout=10,
