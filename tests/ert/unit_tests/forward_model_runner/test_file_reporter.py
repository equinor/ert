...
def test_report_with_successful_start_message_argument(reporter: File) -> None:
    msg = Start(
        ForwardModelStep(
            create_stub_forward_model_step_json(
                argList=["--foo", "1", "--bar", "2"], executable="/bin/sh"
            ),
            0,
        )
    )
...
def test_report_with_failed_start_message_argument(reporter: File) -> None:
    msg = Start(ForwardModelStep(create_stub_forward_model_step_json(), 0)).with_error(
        "massive_failure"
    )
    assert msg.step is not None
    reporter.status_dict = reporter._init_step_status_dict(
...
def test_report_with_successful_exit_message_argument(reporter: File) -> None:
    msg = Exited(ForwardModelStep(create_stub_forward_model_step_json(), 0), 0)
    assert msg.step is not None
    reporter.status_dict = reporter._init_step_status_dict(
        msg.timestamp, "0", [msg.step]
    )
...
def test_report_with_failed_exit_message_argument(reporter: File) -> None:
    msg = Exited(
        ForwardModelStep(
            create_stub_forward_model_step_json(stderr=None, stdout=None), 0
        ),
        1,
...
def test_report_with_running_message_argument(reporter: File) -> None:
    msg = Running(
        ForwardModelStep(create_stub_forward_model_step_json(), 0),
...
def test_dump_error_file_with_stderr(reporter: File) -> None:
    ...
    reporter._dump_error_file(
        ForwardModelStep(
            create_stub_forward_model_step_json(stderr="stderr.out.0"),
            0,
        ),
        "massive_failure",
    )
...
def test_status_file_is_correct(reporter: File) -> None:
    ...
    """
    j_1 = ForwardModelStep(create_stub_forward_model_step_json(name="j_1"), 0)
    j_2 = ForwardModelStep(create_stub_forward_model_step_json(name="j_2"), 1)
