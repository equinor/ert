...
async def test_when_task_fails_evaluator_raises_exception(
    ...
    def snapshot_event_handler(snapshot):
        pass
    ...
    evaluator = EnsembleEvaluator(
        ...
        end_queue=end_queue,
        snapshot_event_handler=snapshot_event_handler,
    )
...
async def test_evaluator_raises_on_invalid_dispatch_event(make_ee_config):
    ...
    def snapshot_event_handler(event):
        pass
...
def evaluator_to_use(
    ...
    async def _evaluator_to_use(
        ...
        evaluator = EnsembleEvaluator(
            ensemble, make_ee_config(use_token=False), end_queue, snapshot_event_handler
        )
...
async def test_restarted_jobs_do_not_have_error_msgs(evaluator_to_use):
    ...
    async with evaluator_to_use(
        snapshot_event_handler=event_queue.put_nowait
    ) as evaluator:
