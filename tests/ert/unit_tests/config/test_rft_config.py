...
def test_that_rft_with_no_matching_well_and_dates_returns_empty_frame(mocker):
    ...
    mocker.patch("builtins.open", lambda filename, mode: rft_file)
...
def test_that_rft_reads_matching_well_and_date(mocker):
    ...
    def mock_rft_file(*args, **kwargs):
        if str(args[0]).endswith("BASE.RFT"):
            return rft_file
        else:
            return original_open(*args, **kwargs)

    mocker.patch("builtins.open", mock_rft_file)
    rft_config = RFTConfig(
        input_files=["BASE.RFT"],
        data_to_read={
            "WELL": {datetime.date(year=2000, month=1, day=1): ["PRESSURE", "SWAT"]}
        },
    )
