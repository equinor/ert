...
@given(_positive_int)
def test_that_make_summary_key_data_parses_region_aquifer_network_keys(number):
...
def test_that_make_summary_key_data_parses_block_keys(i, j, k):
...
@given(_name)
def test_that_make_summary_key_data_parses_well_keys(well):
...
@given(_name)
def test_that_make_summary_key_data_parses_group_keys(name):
...
def test_that_make_summary_key_data_parses_segment_keys(name, number):
...
def test_that_make_summary_key_data_parses_completion_keys(name, i, j, k):
...
@given(_name, _name)
def test_that_make_summary_key_data_parses_local_well_keys(lgr_name, name):
...
@given(_name, _positive_int, _positive_int, _positive_int)
def test_that_make_summary_key_data_parses_local_block_keys(lgr_name, i, j, k):
...
def test_that_make_summary_key_data_parses_other_keys(other, name):
    assert make_summary_key_data(f"{other}:{name}") == SummaryKeyData(keyword=other)


def test_that_make_summary_key_data_raises_for_invalid_keys():
    with pytest.raises(InvalidSummaryKeyError) as e:
        make_summary_key_data("")
    assert str(e.value) == "Invalid summary key "
