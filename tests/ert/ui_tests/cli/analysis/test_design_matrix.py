...
def test_run_poly_example_with_design_matrix(copy_poly_case_with_design_matrix, caplog):
    ...
    with open_storage(storage_path) as storage:
        ...
        params = ensemble.load_parameters_numpy(
            "DESIGN_MATRIX",
            np.flatnonzero(ensemble.get_realization_mask_with_parameters()),
        )
...
def test_design_matrix_on_esmda(experiment_mode, ensemble_name, iterations):
    ...
    with open_storage(storage_path) as storage:
        ...
        for i in range(iterations):
            ...
            coeffs_a = ensemble.load_parameters_numpy(
                "COEFFS_A",
                np.flatnonzero(ensemble.get_realization_mask_with_parameters()),
            )
            ...
            coeffs_b = ensemble.load_parameters_numpy(
                "COEFFS_B",
                np.flatnonzero(ensemble.get_realization_mask_with_parameters()),
            )[0]
            assert values == pytest.approx(coeffs_b, abs=0.0001)
