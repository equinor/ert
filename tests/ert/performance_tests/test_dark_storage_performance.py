...
from ert.dark_storage.endpoints.experiments import get_experiments
...
def get_parameters(storage, ensemble_id1, keyword, poly_ran):
    async def get_params_from_experiment():
        fetched_experiments = get_experiments(storage=storage)
        exp = next(
            exp for exp in fetched_experiments if ensemble_id1 in exp.ensemble_ids
        )
        return [
            param_info["key"]
            for group_info in exp.parameters.values()
            for param_info in group_info
        ]

    parameters_json = run_in_loop(get_params_from_experiment())
    assert (
        len(parameters_json)
...
@pytest.mark.parametrize(
    "function",
    [get_record_parquet, get_record_csv, get_parameters],
)
@pytest.mark.parametrize(
