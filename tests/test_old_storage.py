...
def test_running_storage(tmpdir):
    ensemble_size = 500
    nr_responses = 100
    with tmpdir.as_cwd():
        config = dedent(
...
TIME_MAP time_map
        ...
        run_context = ert.create_ensemble_context(
            "prior", [True] * ert.getEnsembleSize(), 0
        )
        posterior_context = ert.create_ensemble_context(
            "posterior", [True] * ert.getEnsembleSize(), 1
        )

        import time

        start_time = time.time()
        ert.sample_prior(run_context.sim_fs, list(range(ensemble_size)))
        ...
        start_time = time.time()
        facade.load_from_forward_model("prior", [True] * ensemble_size, 0)
        print(f"Time for loading: {time.time() - start_time}")
        es_update = ESUpdate(ert)
        start_time = time.time()
        es_update.smootherUpdate(
            run_context.sim_fs, posterior_context.sim_fs, "prior.run_id"
        )
