        ...
        st.fixed_dictionaries(
            {
                ...
                ConfigKeys.LOG_FILE: file_names,
                ConfigKeys.LOG_LEVEL: log_levels,
                ...
                ConfigKeys.END_DATE: st.builds(
                    lambda d: d.strftime("%d/%m/%Y"),
                    st.dates(min_value=datetime.date(year=1980, month=1, day=1)),
                ),
    ...
    with open(filename, "w+") as config:
        ...
        if ConfigKeys.SETENV in config_dict:
            for statement in config_dict[ConfigKeys.SETENV]:
                config.write(
                    f"{ConfigKeys.SETENV} {statement[ConfigKeys.NAME]} {statement[ConfigKeys.VALUE]}\n"
                )

        if ConfigKeys.ANALYSIS_COPY in config_dict:
            for statement in config_dict[ConfigKeys.ANALYSIS_COPY]:
                config.write(
                    f"{ConfigKeys.ANALYSIS_COPY} {statement[ConfigKeys.SRC_NAME]} {statement[ConfigKeys.DST_NAME]}\n"
                )
        ...
        if ConfigKeys.INSTALL_JOB in config_dict:
            for job in config_dict[ConfigKeys.INSTALL_JOB]:
                config.write(
                    f"{ConfigKeys.INSTALL_JOB} {job[ConfigKeys.NAME]} {job[ConfigKeys.PATH]}\n"
                )

        if ConfigKeys.QUEUE_OPTION in config_dict:
            for setting in config_dict[ConfigKeys.QUEUE_OPTION]:
                config.write(
                    f"{ConfigKeys.QUEUE_OPTION} {setting[ConfigKeys.NAME]} {setting[ConfigKeys.VALUE]}\n"
                )
        ...
        if ConfigKeys.QUEUE_SYSTEM in config_dict:
            config.write(
                f"{ConfigKeys.QUEUE_SYSTEM} {config_dict[ConfigKeys.QUEUE_SYSTEM].name[:-7]}\n"
            )

        if ConfigKeys.LOG_LEVEL in config_dict:
            config.write(
                f"{ConfigKeys.LOG_LEVEL} {config_dict[ConfigKeys.LOG_LEVEL].name[4:]}\n"
            )
