...
def test_repr(minimum_example):
    assert repr(minimum_example).startswith("EnKFMain(ensemble_size")


def test_bootstrap(minimum_example):
    assert bool(minimum_example)
...
def test_default_res_config(minimum_example):
    assert minimum_example.resConfig is not None
    assert minimum_example.siteConfig is not None
    assert minimum_example.analysisConfig is not None
...
def test_observations(minimum_example):
    count = 10
    ...
    minimum_example.getObservations().addObservationVector(observation_vector)
    ...
    observations = minimum_example.getObservations()
...
def test_config(minimum_example):

    assert isinstance(minimum_example.ensembleConfig(), EnsembleConfig)
    assert isinstance(minimum_example.analysisConfig(), AnalysisConfig)
    assert isinstance(minimum_example.getModelConfig(), ModelConfig)
    assert isinstance(minimum_example.siteConfig(), SiteConfig)
    assert isinstance(minimum_example.eclConfig(), EclConfig)

    assert isinstance(minimum_example.getObservations(), EnkfObs)
    assert isinstance(minimum_example.getEnkfFsManager().getCurrentFileSystem(), EnkfFs)

    assert minimum_example.getMountPoint().endswith("/Ensemble")


def test_run_context(minimum_example):
    fs_manager = minimum_example.getEnkfFsManager()
    fs = fs_manager.getCurrentFileSystem()
    iactive = [True] * 10
    iactive[0] = False
    iactive[1] = False
    run_context = minimum_example.create_ensemble_experiment_run_context(
    ...
    rng1 = minimum_example.rng()
    rng1.setState("ABCDEFGHIJ012345")
    d1 = rng1.getDouble()
    rng1.setState("ABCDEFGHIJ012345")
    rng2 = minimum_example.rng()
    d2 = rng2.getDouble()
    assert d1 == d2
