...
def test_init(minimum_example):
    assert minimum_example.model_config.data_root() == os.path.join(
        os.getcwd(), "simple_config"
    )
    assert clib_getenv("DATA_ROOT") == os.path.join(os.getcwd(), "simple_config")

    # This fails with an not-understandable Python error:
    # -----------------------------------------------------------------
    # minimum_example.model_config.set_data_root( "NEW" )
    # assert  minimum_example.model_config.data_root( )  == "NEW"
    # assert  clib_getenv("DATA_ROOT")  == "NEW"

    assert minimum_example is not None

    assert minimum_example.site_config is not None
    assert isinstance(minimum_example.site_config, SiteConfig)

    assert minimum_example.analysis_config is not None
    assert isinstance(minimum_example.analysis_config, AnalysisConfig)

    assert minimum_example.config_path == os.path.join(os.getcwd(), "simple_config")

    assert minimum_example.subst_config["<CONFIG_PATH>"] == os.path.join(
        os.getcwd(), "simple_config"
    )
...
def test_extensive_config(setup_case):
    ...
    assert (
        snake_oil_structure_config["UPDATE_LOG_PATH"] == analysis_config.get_log_path()
    )
...
def test_res_config_dict_constructor(setup_case):
    ...
    cfg_path, cfg_file = os.path.split(os.path.realpath("user_config.ert"))
