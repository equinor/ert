...
def test_single_parameter():
    single_gauss = ert3.stats.Gaussian(0, 1, size=1)
    evaluations = ert3.algorithms.one_at_the_time({"single": single_gauss})

    assert 2 == len(evaluations)
    for idx, parameter_value in enumerate([-CNORM_INV, CNORM_INV]):
