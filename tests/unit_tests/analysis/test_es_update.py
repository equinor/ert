...
def test_update_report(
    ...
    snapshot.assert_match(tabulate(event.data, floatfmt=".3f") + "\n", "update_log")
...
def test_update_report_with_exception_in_analysis_ES(
    ...
    snapshot.assert_match(
        tabulate(error_event.data, floatfmt=".3f") + "\n", "error_event"
    )
...
def test_update_report_with_different_observation_status_from_smoother_update(
    ...
    assert update_event.extra["Active observations"] == str(active)
    assert update_event.extra["Deactivated observations - missing respons(es)"] == str(
        missing
    )
    assert update_event.extra[
        "Deactivated observations - ensemble_std > STD_CUTOFF"
    ] == str(std_cutoff)
    assert update_event.extra["Deactivated observations - outliers"] == str(outliers)
...
def test_update_only_using_subset_observations(
    ...
    update_event = next(e for e in events if isinstance(e, AnalysisCompleteEvent))
    snapshot.assert_match(
        tabulate(update_event.data, floatfmt=".3f") + "\n", "update_log"
    )
