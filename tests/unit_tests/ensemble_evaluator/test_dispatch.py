...
class DummyEventHandler:
    ...
    def __init__(self):
        ...
        self.dispatcher.register_event_handler(ids.EVGROUP_FM_ALL, self.fm)
        self.dispatcher.register_event_handler({ids.EVTYPE_ENSEMBLE_FAILED}, self.fail)

    async def join(self):
        await self.dispatcher.join()

    async def fm(self, event):
        self.mock_fm(event)
