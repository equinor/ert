from pytestqt.qt_compat import qt_api
from qtpy.QtCore import QModelIndex

from ert.ensemble_evaluator.state import (
    REALIZATION_STATE_FINISHED,
    REALIZATION_STATE_RUNNING,
)
from ert.gui.model.real_list import RealListModel
from ert.gui.model.snapshot import NodeRole, SnapshotModel

from .gui_models_utils import finish_snapshot


def test_using_qt_model_tester(qtmodeltester, full_snapshot):
    source_model = SnapshotModel()

    model = RealListModel(None, 0)
    model.setSourceModel(source_model)

    reporting_mode = qt_api.QtTest.QAbstractItemModelTester.FailureReportingMode.Warning
    tester = qt_api.QtTest.QAbstractItemModelTester(  # noqa: F841, prevent GAC
        model, reporting_mode
    )

    source_model._process_full_snapshot(SnapshotModel.prerender(full_snapshot), "0")
    source_model._process_full_snapshot(SnapshotModel.prerender(full_snapshot), "1")

    modified_snapshot = finish_snapshot(full_snapshot)
    source_model._process_full_snapshot(SnapshotModel.prerender(modified_snapshot), "0")
    source_model._process_full_snapshot(SnapshotModel.prerender(modified_snapshot), "1")

    qtmodeltester.check(model, force_py=True)


def test_change_iter(full_snapshot):
    source_model = SnapshotModel()

    model = RealListModel(None, 0)
    model.setSourceModel(source_model)

    reporting_mode = qt_api.QtTest.QAbstractItemModelTester.FailureReportingMode.Warning
    tester = qt_api.QtTest.QAbstractItemModelTester(  # noqa: F841, prevent GC
        model, reporting_mode
    )

    source_model._process_full_snapshot(SnapshotModel.prerender(full_snapshot), "0")

    assert (
        model.index(0, 0, QModelIndex()).data(NodeRole).data.status
        == REALIZATION_STATE_RUNNING
    )

    source_model._process_full_snapshot(SnapshotModel.prerender(full_snapshot), "1")

    model.setIter(1)

    modified_snapshot = finish_snapshot(full_snapshot)
    modified_snapshot._realization_states["0"].update(
        {"status": REALIZATION_STATE_FINISHED}
    )
    source_model._process_full_snapshot(SnapshotModel.prerender(modified_snapshot), "1")

    assert (
        model.index(0, 0, QModelIndex()).data(NodeRole).data.status
        == REALIZATION_STATE_FINISHED
    )
