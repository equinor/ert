...
from ert.job_queue.queue import EVTYPE_ENSEMBLE_CANCELLED, EVTYPE_ENSEMBLE_SUCCESS
...
async def test_empty(mock_driver):
    ...
    assert await sch.execute() == EVTYPE_ENSEMBLE_SUCCESS
...
async def test_max_runtime(realization, mock_driver, caplog):
    ...
    assert result == EVTYPE_ENSEMBLE_SUCCESS
...
async def test_that_long_running_jobs_were_stopped(storage, tmp_path, mock_driver):
    ...
    assert await sch.execute(min_required_realizations=5) == EVTYPE_ENSEMBLE_SUCCESS
