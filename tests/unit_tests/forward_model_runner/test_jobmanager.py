...
assert exec_env["TEST_ENV"] == "123"
    ...
    forward_model = ForwardModelStep.from_config_file(name=None, config_file="EXEC_ENV")

    with open("jobs.json", mode="w", encoding="utf-8") as fptr:
        json.dump(
            ErtConfig(forward_model_steps=[forward_model]).forward_model_data_to_json(
                "run_id"
            ),
            fptr,
        )
...
assert os.environ["TEST_ENV"] == "123"
    ...
    step = ForwardModelStep.from_config_file(name=None, config_file="RUN_ENV")
    with open("jobs.json", mode="w", encoding="utf-8") as fptr:
        json.dump(
            ErtConfig(forward_model_steps=[step]).forward_model_data_to_json("run_id"),
            fptr,
        )
