...
def test_simulation_context_legacy_jobstatus(setup_case, storage):
...
def test_simulation_context(setup_case, storage):
    ...
    for iens in range(size):
        if iens % 2 == 0:
            assert even_ctx.get_job_state(iens) != JobState.COMPLETED
        else:
            assert odd_ctx.get_job_state(iens) != JobState.COMPLETED
    ...
    for iens in range(size):
        if iens % 2 == 0:
            assert even_ctx.get_job_state(iens) != JobState.FAILED
            assert even_ctx.get_job_state(iens) == JobState.COMPLETED
        else:
            assert odd_ctx.get_job_state(iens) != JobState.FAILED
            assert odd_ctx.get_job_state(iens) == JobState.COMPLETED
