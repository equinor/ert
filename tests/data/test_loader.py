...
def test_load_summary_data(facade, monkeypatch):
    ...
    monkeypatch.setattr(loader, "_add_summary_observations", mocked_get_summary_obs)
    monkeypatch.setattr(loader, "_add_summary_data", mocked_get_summary_measured)

    result = loader.load_summary_data(facade, "some_key", "a_random_name")
    mocked_get_summary_obs.assert_called_once_with(
        facade, facade.get_data_key_for_obs_key("some_key"), "a_random_name"
    )
