...
void ensemble_config_add_gen_kw(ensemble_config_type * config , 
                                ...
                                const char * min_std_file     , /* For inflation - can be NULL. */
                                const char * init_file_fmt ) {  /* fmt string for file based initialization (typically NULL) */

  gen_kw_config_type    * node        = gen_kw_config_fscanf_alloc( key , config_file , template_file , min_std_file , init_file_fmt);
...
ensemble_config_type * ensemble_config_alloc(const config_type * config , const ecl_grid_type * grid) {
  ...
  for (i=0; i < config_get_occurences(config , "GEN_KW"); i++) {
    ...
    ensemble_config_add_gen_kw( ensemble_config , key , enkf_outfile , template_file , config_file , min_std_file , init_file_fmt);
  }
