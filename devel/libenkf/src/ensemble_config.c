...
void ensemble_config_add_gen_kw(ensemble_config_type * config , 
  ...
  gen_kw_config_type    * node        = gen_kw_config_alloc_with_options( key , parameter_file , template_file , options , subst_func_pool);
...
ensemble_config_type * ensemble_config_alloc(const config_type * config , const ecl_grid_type * grid , subst_func_pool_type * subst_func_pool) {
  ...
  for (i=0; i < config_get_occurences(config , "GEN_KW"); i++) {
    ...
    ensemble_config_add_gen_kw( ensemble_config , key , enkf_outfile , template_file , parameter_file , tokens , subst_func_pool);
  ...
  */
  ...
  if (config_item_set( config , "SCHEDULE_PREDICTION_FILE")) {
    ...
    stringlist_idel( tokens , 0);
    ensemble_config_add_gen_kw( ensemble_config , key , target_file , template_file , NULL , tokens, subst_func_pool);
    free(target_file);
    free(template_file);
  }
