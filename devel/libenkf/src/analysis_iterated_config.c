/*
   ...
   The file 'analysis_iterated_config.c' is part of ERT - Ensemble based Reservoir Tool. 
...
struct analysis_iterated_config_struct {
  char            * runpath_fmt;
  char            * enspath_fmt;
...
analysis_iterated_config_type * analysis_iterated_config_alloc() {
   analysis_iterated_config_type * config = util_malloc( sizeof * config );    
   config->runpath_fmt    = NULL;
   config->enspath_fmt    = NULL;
   config->storage        = stringlist_alloc_new();
   analysis_iterated_config_set_num_iterations( config , DEFAULT_ANALYSIS_NUM_ITERATIONS );
   return config;
}
...
static void analysis_iterated_config_set_runpath_fmt( analysis_iterated_config_type * config , const char * runpath_fmt) {
  config->runpath_fmt = util_realloc_string_copy( config->runpath_fmt , runpath_fmt );
}

static void analysis_iterated_config_set_enspath_fmt( analysis_iterated_config_type * config , const char * enspath_fmt) {
...
const char * analysis_iterated_config_iget_enspath_fmt( analysis_iterated_config_type * config , int iter) {
  if (config->enspath_fmt != NULL) {
    char * enspath_fmt = util_alloc_sprintf( config->enspath_fmt , iter );
    stringlist_append_owned_ref( config->storage , enspath_fmt );
    return enspath_fmt;
  } else
    return NULL;
}
...
void analysis_iterated_config_add_config_items( config_type * config ) {
  config_add_key_value( config , ITERATED_ENSPATH_KEY , false , CONFIG_STRING);
  config_add_key_value( config , ITERATED_RUNPATH_KEY , false , CONFIG_STRING);
}


void analysis_iterated_config_init(analysis_iterated_config_type * iter_config , const config_type * config) {
  if (config_item_set( config , ITERATED_ENSPATH_KEY ))
    analysis_iterated_config_set_enspath_fmt( iter_config , config_get_value( config , ITERATED_ENSPATH_KEY ));
  
  if (config_item_set( config , ITERATED_RUNPATH_KEY ))
    analysis_iterated_config_set_runpath_fmt( iter_config , config_get_value( config , ITERATED_RUNPATH_KEY ));
}
