...
void relperm_inter_init__(const char * _config_file, const int * config_file_len, const int * ens_size, const int * n_relperm) {
  int iens;
  char * config_file = util_alloc_cstring(_config_file, config_file_len);
  RELPERM_CONFIG = relperm_config_fscanf_alloc(config_file);
