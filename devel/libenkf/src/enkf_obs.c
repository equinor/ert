...
enkf_obs_type * enkf_obs_fscanf_alloc(const char * config_file,  const history_type * hist, const ecl_sum_type * refcase , ensemble_config_type * ensemble_config, double std_cutoff) {
  ...
  /** Handle HISTORY_OBSERVATION instances. */
  {
    ...
    for(int hist_obs_nr = 0; hist_obs_nr < num_hist_obs; hist_obs_nr++)
    {
      ...
      obs_vector_type * obs_vector = obs_vector_alloc_from_HISTORY_OBSERVATION(hist_obs_conf , hist , refcase , ensemble_config , std_cutoff);
  ...
  /** Handle SUMMARY_OBSERVATION instances. */
  {
    ...
    for(int sum_obs_nr = 0; sum_obs_nr < num_sum_obs; sum_obs_nr++)
    {
      ...
      obs_vector_type * obs_vector = obs_vector_alloc_from_SUMMARY_OBSERVATION(sum_obs_conf , hist , refcase , ensemble_config);
