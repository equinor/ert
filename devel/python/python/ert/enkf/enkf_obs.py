...
class EnkfObs(BaseCClass):
    ...
    def __getitem__(self , index):
        if isinstance(index, str):
            return self.getObservationsVector(index)
        elif isinstance(index,int):
            return self.igetObservationsVector(index)
        else:
            raise TypeError("[] operator must have string or integer")
    ...
    def igetObservationsVector(self, index):
        """ @rtype: ObsVector """
        assert isinstance(index, int)
        if index < 0:
            raise ValueError("Index:%d - invalid" % index)
        elif index >= len(self):
            raise IndexError("Index:%d - invalid" % index)
            
        return EnkfObs.cNamespace().iget_vector(self, index).setParent(self)
