...
def createEclipsePage(configPanel, parent):
    ...
    r.initialize = lambda ert : ert.ecl_config().get_eclbase()
    r.getter = lambda ert : ert.ecl_config().get_eclbase()
    r.setter = lambda ert, value : ert.set_eclbase(str(value))

    r = configPanel.addRow(PathChooser(parent, "Data file", "config/eclipse/data_file", show_files=True))
    r.initialize = lambda ert : ert.ecl_config().get_data_file()
    r.getter = lambda ert : ert.ecl_config().get_data_file()
    r.setter = lambda ert, value : ert.set_datafile(str(value))

    r = configPanel.addRow(PathChooser(parent, "Grid", "config/eclipse/grid", show_files=True))
    r.initialize = lambda ert : ert.ecl_config().get_gridfile()
    r.getter = lambda ert : ert.ecl_config().get_gridfile()
    r.setter = lambda ert, value : ert.ecl_config().set_gridfile(str(value))

    r = configPanel.addRow(PathChooser(parent, "Schedule file" , "config/eclipse/schedule_file" , show_files = True))
    r.initialize = lambda ert : ert.ecl_config().get_schedule_file()
    r.getter = lambda ert : ert.ecl_config().get_schedule_file()
    r.setter = lambda ert, value : ert.ecl_config().set_schedule_file( str(value))


    r = configPanel.addRow(PathChooser(parent, "Init section", "config/eclipse/init_section", show_files=True))
    r.initialize = lambda ert : ert.ecl_config().get_init_section()
    r.getter = lambda ert : ert.ecl_config().get_init_section()
    r.setter = lambda ert, value : ert.ecl_config().set_init_section( str(value))


    r = configPanel.addRow(PathChooser(parent, "Refcase", "config/eclipse/refcase", show_files=True))
    r.initialize = lambda ert : ert.ecl_config().get_refcase_name()
    r.getter = lambda ert : ert.ecl_config().get_refcase_name()
    r.setter = lambda ert, value : ert.ecl_config().load_refcase( str(value))

    r = configPanel.addRow(PathChooser(parent, "Schedule prediction file", "config/eclipse/schedule_prediction_file", show_files=True))
    r.initialize = lambda ert : ert.get_schedule_prediction_file()
    r.getter = lambda ert : ert.get_schedule_prediction_file()
    r.setter = lambda ert, value : ert.set_schedule_prediction_file( ert.nonify( value ))
    ...
    def get_data_kw(ert):
        ...
        result = []
        for key, value, doc_string in subst_list:
            result.append([key, value]) #+ " (" + doc_string + ")"

        return result

    r.initialize = get_data_kw
    r.getter = get_data_kw

    def add_data_kw(ert, list_of_keywords):
        ert.clearDataKW()

        for keyword in list_of_keywords:
            ert.addDataKW(keyword[0], keyword[1])
    ...
    r.getter = lambda ert : ert.ecl_config().get_static_kw_list()

    def add_static_kw(ert, list_of_keywords):
        ert.ecl_config().clear_static_kw()

        for keyword in list_of_keywords:
            ert.ecl_config().add_static_kw(keyword)

    r.setter = add_static_kw
    r.initialize = lambda ert : ert.ecl_config().get_static_kw_list()
