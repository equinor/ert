...
/**
   ...
   sched_kw_wconinje implementation is asking for well names.
...
static sched_kw_wconinje_type * sched_kw_wconinje_alloc(bool alloc_untyped)
{
  ...
  if (alloc_untyped)
    kw->untyped_kw = sched_kw_untyped_alloc("WCONINJE");  /* Hardcoded ... */
  else
    kw->untyped_kw = NULL;
  return kw;
}
...
sched_kw_wconinje_type *  sched_kw_wconinje_fread_alloc(FILE *stream) {
  sched_kw_wconinje_type * kw = sched_kw_wconinje_alloc(false);
