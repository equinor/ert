...
void perturb_rates( void * void_kw , int restart_nr , void * arg) {
...
void config_init(config_type * config ) {
  ...
  config_item_set_argc_minmax(item , 3 , 3 , (const config_item_types[3]) { CONFIG_STRING,             /* Group name */
                                                                              CONFIG_STRING ,          /* Phase */
  ...
  config_item_set_argc_minmax(item , 5 , 5 , (const config_item_types[5]) { CONFIG_STRING,           /* Group name */
                                                                              CONFIG_STRING ,        /* Well name */
                                                                              CONFIG_STRING ,        /* Phase */
                                                                              CONFIG_FLOAT  ,        /* Corr_length (days) */
                                                                              CONFIG_EXISTING_FILE});/* File with mean , std shift */
  config_item_set_indexed_selection_set( item , 2 , 3 , (const char *[3]) { "OIL" , "GAS" , "WATER"});
}
...
void load_groups( const config_type * config , const sched_file_type * sched_file , hash_type * group_rates , const time_t_vector_type * time_vector ) {
  ...
  for (i=0; i < config_get_occurences( config , "GROUP_RATE" ); i++) {
    ...
    const char * min_max_file = config_iget( config , "GROUP_RATE" , i , 2 );

    group_rate_type * group_rate = group_rate_alloc( time_vector , group_name , phase_string , min_max_file );
    char * key = util_alloc_sprintf("%s:%s" , group_name , get_phase_name( group_rate_get_phase( group_rate )));
  ...
  for (i=0; i < config_get_occurences( config , "WELL_RATE" ); i++) {
    ...
    char * group_key = util_alloc_sprintf("%s:%s" , group_name , get_phase_name( well_rate_get_phase( well_rate )));
  ...
  config_parse(config , argv[1] , "--" , NULL , "DEFINE" , NULL , false , true );
  {
    ...
    
    {
      ...
      for (i = 0; i < num_realizations; i++) {
        ...
        sched_file_type * sched_file = sched_file_parse_alloc( sched_file_name , start_date );
        sample( group_rates );
        sched_file_update( sched_file , WCONHIST , perturb_rates , group_rates );
