...
**Non-updatable** configurations are not updated. Only strategies present in
the parameters are shown, and ``None`` is shown when there are no parameter
configurations.
...
read-only and does not change update behavior or require configuration
changes.
