...
Site configuration
~~~~~~~~~~~~~~~~~~~~~
Ert can have some options set by plugins through
...
otherwise is specified explicitly in the user's configuration file:

.. code-block:: python

    import ert
    from ert.config.queue_config import LsfQueueOptions
    from ert.plugins.plugin_manager import hook_specification, ErtRuntimePlugins
    @hook_implementation
    @ert.plugin(name="my_plugin")
    def site_configurations() -> ErtRuntimePlugins:
        queue_options = LsfQueueOptions(
            lsf_resource="select[x86_64Linux]",
            lsf_queue="default",
            max_running=10000,
            submit_sleep=1,
        )
...
Some Ert configuration options cannot be set via the user configuration file, and
must be set via the site_configuration plugin, e.g., ``prioritize_private_ip_address``.

.. _prioritize_private_ip_address:
.. topic:: prioritize_private_ip_address

    ``prioritize_private_ip_address`` is used to specify if the IP address
    ...
    between Ert and forward models. This defaults to FALSE, meaning using the
    public IP address of the machine running Ert.
