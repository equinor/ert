...
Keyword requires a filepath to the simulation file. Filepath should be relative to the
runpath and can have extensions `.parquet` or `.csv`.

All rows in the file will be loaded.

SEISMIC keyword can be repeated multiple times to load more than one file.
