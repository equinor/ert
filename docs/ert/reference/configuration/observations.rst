...
``OBS_FILE`` could be provided directly or wildcard could be used. In that case, the
same rules as for the :ref:`SEISMIC <seismic>` apply. All the filenames matching the
...
during the update step. Observations located outside the polygon are deactivated. If
...
configuration file, the same as ``OBS_FILE``. File must be written in any format
supported by `xtgeo <https://xtgeo.readthedocs.io/en/stable/api-points-polygons.html#xtgeo.polygons_from_file>`_.
...
- Directly specifying :ref:`SEISMIC <seismic>` keyword allows for full control over
  fetched data.
- If ``SEISMIC`` keyword is not used, results would be fetched from
  ``share/results/tables`` directory in runpath. Files with the same names as
  observation files would be loaded.
