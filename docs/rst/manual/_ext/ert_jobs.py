...
class ForwardModelDocumentation:
    def __init__(
...
class ErtForwardModelDocumentation(SphinxDirective):
    ...
    def _divide_into_categories(jobs):
        ...
        for job_name, docs in jobs.items():
            ...
            category = docs.get(
                ErtForwardModelDocumentation._CATEGORY_KEY,
                ErtForwardModelDocumentation._CATEGORY_DEFAULT,
            )
            ...
            categories[main_category].append(
                ForwardModelDocumentation(
                    name=job_name,
                    category=category,
                    job_source=docs.get(
                        ErtForwardModelDocumentation._SOURCE_PACKAGE_KEY,
                        ErtForwardModelDocumentation._SOURCE_PACKAGE_DEFAULT,
                    ),
                    description=docs.get(
                        ErtForwardModelDocumentation._DESCRIPTION_KEY,
                        ErtForwardModelDocumentation._DESCRIPTION_DEFAULT,
                    ),
                    job_config_file=docs.get(
                        ErtForwardModelDocumentation._CONFIG_FILE_KEY,
                        ErtForwardModelDocumentation._CONFIG_FILE_DEFAULT,
                    ),
                    examples=docs.get(
                        ErtForwardModelDocumentation._EXAMPLES_KEY,
                        ErtForwardModelDocumentation._EXAMPLES_DEFAULT,
                    ),
                )
            )
    ...
    def _create_forward_model_section_node(self):
        node = _create_section_with_title(
            section_id="ert-forward-models", title="Forward models"
        )
        _parse_string_list(self.content, node, self.state)
        return node

    def _generate_job_documentation(self):
        pm = ErtPluginManager()
        jobs = pm.get_documentation_for_jobs()

        job_categories = ErtForwardModelDocumentation._divide_into_categories(jobs)

        main_node = self._create_forward_model_section_node()
    ...
    def run(self):
        if sys.version_info.major >= 3:
            ...
            jobs_node = _create_section_with_title(
                section_id="ert-forward-models", title="Forward models"
            )
