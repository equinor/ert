...
``algorithm``: the algorithm used for the optimisation
...
``realizations``: list of realizations to use in optimisation ensemble. Typically, this is a
...
``point.json``: file generated by the Everest optimisation experiment. It contains the list of control variables with
...
``distance``: file required for the optimisation experiment
to succeed.
...
Optimisation environment definition
-----------------------------------
...
``output_folder``: folder path where the optimisation information will be written.

``random_seed``: integer used to make optimisation experiments reproducible
...
Before we are able to start the optimisation experiment we need to create the script for the custom job
