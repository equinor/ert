...
class NewSnapshot:
    ...
    @classmethod
    def _from_nested_dict(cls, data: Mapping[Any, Any]) -> "NewSnapshot":
    ...
    def merge_event(self, event: "NewSnapshot") -> None:
        self._merge(event)

    def merge(self, update_as_nested_dict: Mapping[str, Any]) -> None:
        self._merge(NewSnapshot._from_nested_dict(update_as_nested_dict))

    def _merge(self, other: "NewSnapshot") -> "NewSnapshot":
    ...
    def update_from_cloudevent(
        self, event: CloudEvent, based_on: Optional["NewSnapshot"] = None
    ) -> "NewSnapshot":
        e_type = event["type"]
        e_source = event["source"]
        timestamp = event["time"]
        if based_on is None:
            based_on = NewSnapshot()
        if e_type in ids.EVGROUP_REALIZATION:
            ...
            if e_type == ids.EVTYPE_REALIZATION_TIMEOUT:
                for (
                    forward_model_id,
                    forward_model,
                ) in based_on.get_forward_models_for_real(
                    ...
                    if (
                        ...
                        forward_model_idx = (real_id, forward_model_id)
                        if forward_model_idx not in based_on._forward_model_states:
            ...
            other_partial = self._from_nested_dict(event.data)
