...
def capture_specific_warning(
    warning_class: type[Warning], propagate_warning_method: Callable[[Any], None]
    ...
    def _custom_warning_handler(
        ...
        if issubclass(category, warning_class):
            propagate_warning_method(WarningEvent(msg=str(message)))
        else:
