...
class ErtClient:
    ...
    def stop_server(self, retries: int = 5) -> bool:
        ...
        while status_code != httpx.codes.OK and retries > 0:
            status_code = self._request(
                "POST",
                f"{_EXPERIMENT_RUNS}/{EverEndpoints.STOP}",
                auth=self._auth,
            ).status_code
            retries -= 1
    ...
    def iter_events(
        ...
        url = (
            ...
            + f"{_EXPERIMENT_RUNS}/{EverEndpoints.EVENTS}/{experiment_id}"
        )
    ...
    def _connect_with_retry(
        ...
        for attempt in range(1, _WEBSOCKET_CONNECT_RETRIES + 1):
            try:
                ...
                delay = next(delays)
                logger.warning(
                    "Transient error connecting to WebSocket event stream at "
    ...
    def _experiment_runs_get(self, path: str) -> httpx.Response:
        return _checked(
            self._request("GET", f"{_EXPERIMENT_RUNS}/{path}", auth=self._auth)
        )
