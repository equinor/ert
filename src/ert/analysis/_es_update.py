...
def update_3D_field_with_distance_esmda_unoptimal(
    ...
    assert X_prior.shape[1] == nreal, (
        f"Mismatch between X_prior dimension {X_prior.shape[1]} and nparam {nreal}"
    )
...
def update_2D_field_with_distance_esmda(
    ...
    calc_max_number_of_layers_per_batch_for_distance_localization(
        nx, ny, 1, nobs, 1, bytes_per_float=8
    )  # use float64
