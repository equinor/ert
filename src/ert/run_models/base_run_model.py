...
class BaseRunModel(BaseModelWithContextSupport, ABC):
    ...
    def api(self) -> BaseRunModelAPI:
        return BaseRunModelAPI(
            ...
            runpath_format_string=str(self.runpath_file),
        ...
        for run_arg in run_args:
            realizations.append(
                Realization(
                    ...
                    run_arg=run_arg,
                    num_cpu=self.queue_config.preferred_num_cpu,
                    job_script=self.queue_config.job_script,
                    realization_memory=self.queue_config.realization_memory,
                )
            )
        return EEEnsemble(
            realizations,
