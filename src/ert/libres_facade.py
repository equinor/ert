...
from res.enkf import EnKFMain, EnkfNode, EnkfVarType, ErtImplType, ResConfig, RunContext
...
class LibresFacade:  # pylint: disable=too-many-public-methods
    ...
    def export_field_parameter(
        ...
        EnkfNode.exportMany(
            ...
            filepath + "." + ext,
            file_system,
            np.arange(0, self.ensemble_size),
        )
    ...
    def get_measured_data(  # pylint: disable=too-many-arguments
        ...
        return MeasuredData(facade, keys, index_lists, load_data, case_name)
    ...
    def get_ensemble_parameters(self) -> List[Tuple[str, str, str]]:
        params = []
        ens_config = self._enkf_main.ensembleConfig()
        ...
        for key in keylist:
            ...
            if node.getVariableType() == EnkfVarType.PARAMETER:
                if impl_type == ErtImplType.GEN_KW:
                    ...
                    params.append(
                        (key, impl_type.name, kw_config_model.getKeyWords().strings)
                    )
                elif impl_type == ErtImplType.GEN_DATA:
                    gen_data_config = node.getDataModelConfig()
                    data_size = gen_data_config.get_initial_size()
                    params.append(
                        (key, impl_type.name, [str(item) for item in range(data_size)])
                    )
                else:
                    params.append((key, impl_type.name, []))
    ...
    def add_workflow_job(self, name: str, job: str) -> None:
        self._enkf_main.getWorkflowList().addJob(name, job)

    def run_workflow_job(self, job: "WorkflowJob", parameters: List[str]) -> None:
        job.run(self, parameters)
