...
class MultipleDataAssimilation(BaseRunModel):
    ...
    def runSimulations(
        ...
        starting_iteration = (
            self.prior_ensemble.iteration if self.prior_ensemble is not None else 0
        )
        ...
        active_realizations = [
            i
            for i, val in enumerate(self._simulation_arguments["active_realizations"])
            if val
        ]
        self.ert().sample_prior(prior_fs, active_realizations)

        prior_ensemble = self.prior_ensemble
        if prior_ensemble is None:
            prior_ensemble = self._storage.create_ensemble(
                self._experiment_id,
                name=case_format % starting_iteration,
                ensemble_size=self._ert.getEnsembleSize(),
            )

            self._ert.sample_prior(prior_ensemble, active_realizations)
