...
info = (
    "Internalizes run templates in to the storage"
    " and migrate template GEN_KW feature into RUN_TEMPLATE"
)
        ...
        with open(experiment / "parameter.json", "w", encoding="utf-8") as fout:
            for param in parameters_json.values():
                if param["_ert_kind"] == "GenKwConfig":
                    # removing deprecated forward_init_file
                    param.pop("forward_init_file", None)
                    src = param.pop("template_file", None)
                    dst = param.pop("output_file", None)
                    # if src is None or dst is None:
                    #     continue
                    # incoming_template = Path(src)
                    # idx = len(templates_abs)
                    # template_file_path = templates_path / (
                    #     f"{incoming_template.stem}_{idx}{incoming_template.suffix}"
                    # )
                    # shutil.copyfile(incoming_template, template_file_path)
                    # templates_abs.append(
                    #     (str(template_file_path.relative_to(experiment)), dst)
                    # )
