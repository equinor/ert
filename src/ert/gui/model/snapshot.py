    ...
    def _add_partial_snapshot(self, partial: PartialSnapshot, iter_: int):
        ...
        with ExitStack() as stack:
            ...
            for real_id in partial.get_real_ids():
                ...
                for real_forward_model_id, color in metadata.aggr_job_status_colors.get(
                    real_id, {}
                ).items():
    ...
    def _add_snapshot(self, snapshot: Snapshot, iter_: int):
        ...
        snapshot_tree = IterNode(
            ...
            data=IterNodeData(
                ...
                sorted_forward_model_step_ids_by_realization_id=metadata.sorted_forward_model_ids,
            ),
        )
        for real_id in snapshot_tree.data.sorted_realization_ids:
            ...
            real_node = RealNode(
                ...
                data=RealNodeData(
                    status=real.status,
                    active=real.active,
                    forward_model_step_status_color_by_id=metadata.aggr_job_status_colors[
                        real_id
                    ],
