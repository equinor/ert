...
class ErtMainWindow(QMainWindow):
    ...
    def _build_plot_window(self) -> QWidget:
        if self._plot_window:
            ...
            self.central_layout.removeWidget(self._plot_window)
    ...
    def select_central_widget(self, index_name: str) -> None:
        ...
        if index_name == EXPERIMENT_STATUS:
            run_dialog_name = self._first_run_dialog_name()
            if run_dialog_name is None:
                # No run exists yet, so there is no status page to show. Keep the
                # current page instead of hiding every panel and blanking the UI.
                return
            index_name = run_dialog_name
