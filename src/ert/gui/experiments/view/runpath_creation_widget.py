...
class RunpathCreationProgressBar(QWidget):
    ...
    def handle_event(self, event: RunPathCreationEvent) -> None:
        if event.sub_type == "StartingTotalRunPathCreation":
            ...
            if event.runpath_number is not None:
                self._bar.setValue(event.runpath_number)
                self._label.setText(
                    f"{event.runpath_number} / {self._total} runpaths created"
                )
