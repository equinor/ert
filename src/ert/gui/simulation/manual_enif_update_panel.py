...
from ert.mode_definitions import MANUAL_ENIF_UPDATE_MODE
...
class ManualEnIFUpdatePanel(ExperimentConfigPanel):
    def __init__(
        self,
        ensemble_size: int,
        run_path: str,
        ...
        super().__init__(ManualUpdateEnIF)
        self.setObjectName("Manual_update_panel")

        layout = QFormLayout()
        lab = QLabel(ManualUpdateEnIF.name())
    ...
    def experimentTypeChanged(self, w: QWidget) -> None:
        if isinstance(w, ManualEnIFUpdatePanel):
            self._realizations_from_fs()
