  ...
  $$self.$$.update = () => {
    ...
    256) {
      $$invalidate(7, kdeCharts = () => {
        const kde = (ds, bandwidth2, kernelFunction, numPoints2) => {
          const domainX = extent(ds);
          const minX = domainX[0];
          const maxX = domainX[1];
          const spanX = maxX - minX;
          const step = spanX / (numPoints2 - 1);
          const xValues = Array.from({ length: numPoints2 }, (_, i) => minX + i * step);
          const kdeValues = xValues.map((x2) => {
            const sum = ds.reduce(
              (prev2, current) => {
                const diffN = (x2 - current) / bandwidth2;
                return prev2 + kernelFunction(diffN);
              },
              0
            );
            return 1 / (ds.length * bandwidth2) * sum;
          });
          const domainY = extent(kdeValues);
          return {
            kdeValues: xValues.map((x2, i) => [(x2 - minX) / spanX, kdeValues[i]]),
            domainY,
            domainX
          };
        };
...
//# sourceMappingURL=index-v1G1gyHW.js.map
