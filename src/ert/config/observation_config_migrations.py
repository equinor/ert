...
class _GeneralObservationChange:
    source_observation: GeneralObservation
    declaration: str
    restart: int
    lines: list[str]
...
class _SummaryObservationChange:
    source_observation: SummaryObservation
    declaration: str
    date: datetime
    lines: list[str]
...
def remove_refcase_and_time_map_dependence_from_obs_config(
    ...
    for gen_obs in general_observations:
        restart = _get_restart(
            gen_obs, gen_obs.name, obs_time_list, refcase is not None
        )
...
def _handle_history_observation(
    ...
    if refcase is None:
        raise ObservationConfigError.with_context(
            "REFCASE is required when using HISTORY_OBSERVATION", summary_key
        )
