...
class ScalarParameters(ParameterConfig):
    ...
    def save_updated_parameters_and_copy_remaining(
        ...
        df = self.load_parameters(source_ensemble, group, np.array(realizations))
    ...
    def should_use_log_scale(self, group_name: str, param_name: str) -> bool:
        for param in self.groups[group_name]:
    ...
    def write_to_runpath(
        ...
        """
        df = (
            ensemble.load_parameters_scalar(self.name, np.array([real_nr]))
            .filter(pl.col("realization") == real_nr)
