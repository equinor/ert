...
class GenDataConfig(ResponseConfig):
    ...
    def __post_init__(self) -> None:
        if isinstance(self.report_steps_list, list):
            self.report_steps = list(set(self.report_steps_list))

    @classmethod
    def from_config_list(cls, config_list: List[List[str]]) -> Self:
        names = []
        input_files = []
        index = []
        for gen_data in config_list:
            ...
            index.append(report_steps)
        return cls(
            name="gen_data",
            names=names,
            input_files=input_files,
            report_steps_list=index,
        )
    ...
    def read_from_file(self, run_path: str, _: int) -> xr.Dataset:
        ...
        datasets = []
        for filename_fmt, report_steps in zip(self.input_files, self.report_steps_list):
            errors = []
            datasets = []
            ...
            if errors:
                raise ValueError(
                    f"Error reading GEN_DATA: {self.name}, errors: {errors}"
                )
