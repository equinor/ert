...
def parse(filename: str) -> ConfContent:
    ...
    with open(filepath, encoding="utf-8") as f:
        return _validate_conf_content(_parse_content(f.read(), filename))
    ...
    for decl in inp:
        if decl[0] == ObservationType.HISTORY:
            ...
            if len(decl) != 3:
                raise _unknown_declaration_error(decl)
            result.append(
                (decl[0], decl[1], _validate_gen_obs_values(decl[1], decl[2]))
            )
        else:
            raise _unknown_declaration_error(decl)
    return result
...
def _validate_history_values(
    ...
    for key, value in inp.items():
        if key in ["ERROR", "ERROR_MIN"]:
            try:
                if key == "ERROR":
                    error = pos_float(value, key)
                else:
                    error_min = pos_float(value, key)
            except TypeError as err:
                raise _conversion_error(key, value, "float") from err
        elif key == "ERROR_MODE":
...
def _validate_summary_values(
    ...
    for key, value in inp.items():
        if key == "RESTART":
            date_dict["RESTART"] = pos_int(value, key)
        elif key in ["ERROR", "ERROR_MIN", "DAYS", "HOURS", "VALUE"]:
            if key == "ERROR":
                error = pos_float(value, key)
            elif key == "DAYS":
                date_dict["DAYS"] = pos_float(value, key)
            elif key == "HOURS":
                date_dict["HOURS"] = pos_float(value, key)
            elif key == "VALUE":
                observation_value = validate_float(value, key)
            else:
                error_min = pos_float(value, key)
        elif key == "ERROR_MODE":
            error_mode = validate_error_mode(value)
        elif key == "SEGMENT":
            segment.append(_validate_segment_dict(key, value))
        elif key == "KEY":
            summary_key = value
        elif key == "DATE":
...
def _validate_gen_obs_values(
    ...
    for key, value in inp.items():
        if key == "RESTART":
            output["RESTART"] = pos_int(value, key)
        elif key in ["ERROR", "ERROR_MIN", "DAYS", "HOURS", "VALUE"]:
            if key == "VALUE":
                output[str(key)] = validate_float(value, key)
            else:
                output[str(key)] = pos_float(value, key)
...
def pos_float(val: str, key: FileContextToken):
...
def pos_int(val: str, key: FileContextToken):
    try:
        v = int(val)
    except ValueError as err:
        raise _conversion_error(key, val, "int") from err
