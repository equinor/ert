...
class ObjectiveFunctionConfig(BaseModel, extra="forbid"):
    ...
    type: Literal["mean", "stddev"] = Field(
        ...
        description=dedent(
            ...
            the only values supported are "mean" and "stddev", which calculate
            the mean (default) and the negative of the standard deviation over
            the realizations, respectively. The negative of the standard
            deviation is used, since in general the aim is to minimize the
            standard deviation as opposed to the mean, which is preferred to be
            maximized.
            """
        ),
    )
