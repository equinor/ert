...
class _ControlVariable(BaseModel):
    ...
    name: Annotated[str, AfterValidator(no_dots_in_string)] = Field(
        description=dedent(
            ...
            The name of the variable must be unique within a control group, if a
            unique `index` field is defined.
            """
        ),
    )
    ...
    enabled: bool | None = Field(
        ...
        description=dedent(
            ...
            If not set, the value of `enabled` of the control group is used.
            """
        ),
    )
    scaled_range: Annotated[tuple[float, float] | None, AfterValidator(valid_range)] = (
        Field(
            ...
            Internally variables are scaled from their minimal and maximum
...
class ControlVariableConfig(_ControlVariable):
    ...
    index: NonNegativeInt | None = Field(
        ...
        description=dedent(
            """
            A Non-negative integer value to distinguish variables with same name
            within a control group.

            Index should be given either for all of the variables or for none of
            them.
            """
        ),
    )
