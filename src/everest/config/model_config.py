...
class ModelConfig(BaseModel, extra="forbid"):
    realizations: list[NonNegativeInt] = Field(
        description=dedent(
            ...
            This should be a list of realization numbers, indicating which
    ...
    realizations_weights: list[float] = Field(
        ...
        description=dedent(
            ...
            If provided, the weight of each realization is equal to one divided
            by the number or realizations. If provided, the list must consist of
            one weight for each realization (as determined by the `realizations`
            field).
            """
        ),
    )
