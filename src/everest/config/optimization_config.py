...
class OptimizationConfig(BaseModel, extra="forbid"):
    ...
    options: list[str] | dict[str, Any] | None = Field(
        ...
        description=dedent(
            ...
            dictionary with values are passed as keyword arguments to the
            optimizer.
    ...
    cvar: CVaRConfig | None = Field(
        ...
        description=dedent(
            ...
            When this section is present Everest will use Conditional Value at
            Risk (CVaR) to minimize risk. Effectively this means that at each
    ...
    max_iterations: int | None = Field(
        ...
        description=dedent(
            ...
            evaluations. This option is passed through unchanged, and its exact
    ...
    min_pert_success: int | None = Field(
        ...
        description=dedent(
            ...
            1. For each realization, a gradient is calculated from a number of
               perturbations (see the `perturbation_num` option). For each
               realization, `perturbation_num` objective functions are evaluated
               with perturbed controls. The calculation of the gradient for a
               single realization is considered successful if at least
               `min_pert_success` forward model runs for that realization are
               successful. The overall gradient can be calculated if the number
               of successful realization gradients is sufficient (see the
               `min_realizations_success` option). If not, the optimization
    ...
    min_realizations_success: int | None = Field(
        ...
        description=dedent(
            ...
            To successfully calculate the aggregated objective function, the
            ...
            successful in terms of the gradient calculation is determined by
            `min_realizations_success` and `min_pert_success` success. See the
            documentation of the `min_pert_success` field for more details.
            ...
            this and will adjust the value to be equal to one.
            """
        ),
    )
    ...
    speculative: bool = Field(
        ...
        description=dedent(
            ...
            resources if the gradient is not used, it saves clock time if it is
            used, since it was already calculated in parallel with a previous
            function.
            ...
            - Running an optimization with or without this disabled will yield
              different results, since the perturbed variables will differ
              because different (longer) sequences of random numbers will be
              generated.
    ...
    parallel: bool = Field(
        ...
        description=dedent(
            ...
            By default Everest will evaluate a single function and gradient
            evaluation at a time. In case of gradient-free optimizers this can
            be highly inefficient, since these tend to need many independent
            function evaluations at each iteration. By setting parallel to True,
