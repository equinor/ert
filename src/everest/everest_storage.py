...
class BatchStorageData:
    ...
    def __init__(self, path: Path, ert_ensemble: Ensemble | None):
        ...
        self._experiment_path = self._path / "../../../optimizer"
        self._objective = self._read_df_if_exists(
            self._experiment_path / "objective_functions.parquet"
        )
        self._objective_names = (
            self._objective["objective_name"].to_list()
            if self._objective is not None
            else []
        )

        self._controls = self._read_df_if_exists(
            self._experiment_path / "controls.parquet"
        )
        self._controls_names = (
            self._controls["control_name"].to_list()
            if self._controls is not None
            else []
        )

        self._constraints = self._read_df_if_exists(
            self._experiment_path / "nonlinear_constraints.parquet"
        )
        self._constraints_names = (
            self._constraints["constraint_name"].to_list()
            if self._constraints is not None
            else []
        )

        self._ert_storage_df = self.get_ert_storage_df()

    def get_ert_storage_df(self) -> pl.DataFrame | None:
        if self._ert_ensemble is None:
            return None
...
class EverestStorage:
    def __init__(
        self,
        output_dir: Path,
        ert_experiment: Experiment | None,
    ) -> None:
    ...
    def on_batch_evaluation_finished(
        ...
        for batch_id, batch_dict in batch_dicts.items():
            ...
            ert_ens = (
                self._ert_experiment.get_ensemble_by_name(f"batch_{batch_id}")
                if self._ert_experiment is not None
                else None
            )
            with open(
