...
class UnpackedFunctionResults(TypedDict):
...
class UnpackedGradientResults(TypedDict):
...
def _enforce_dtypes(df: pl.DataFrame) -> pl.DataFrame:
    ...
    df = df.cast(
        {colname: dtype for colname, dtype in dtypes.items() if colname in df.columns}
    )
...
def _ropt_to_df(
    ...
    *,
    ...
    df = results.to_polars(values).select(select + values)
    df = _rename_ropt_df_columns(df)
    return _enforce_dtypes(df)


def _unpack_function_results(results: FunctionResults) -> UnpackedFunctionResults:
...
def _unpack_gradient_results(results: GradientResults) -> UnpackedGradientResults:
    logger.debug("Storing gradient results")
    have_perturbed_constraints = results.evaluations.perturbed_constraints is not None
    ...
    if batch_objective_gradient is not None:
        objective_names = batch_objective_gradient["objective_name"].unique().to_list()
...
def unpack_ropt_results(
    ...
    for item in results:
        ...
        if isinstance(item, FunctionResults):
            eval_results = _unpack_function_results(item)
            batch_dicts[item.batch_id].update(eval_results)

        if isinstance(item, GradientResults):
            gradient_results = _unpack_gradient_results(item)
    ...
    for batch_id, batch_dict in batch_dicts.items():
        batch_dataframes[batch_id] = {
            ...
            "batch_constraint_gradient": batch_dict.get("batch_constraint_gradient"),
        }
