...
class EverestMainWindow(QMainWindow):
    ...
    def __init__(self, config: EverestConfig):
        ...
        self.ert_config = ErtConfig(**everest_to_ert_config_dict(config))
        self.config_file = config.config_file

        self.setWindowTitle(
            f"Everest - {self.config_file} - {find_ert_info()} - {get_trace_id()[:8]}"
        )
...
class EverestMainWindowOLD(QMainWindow):
    close_signal = Signal()
