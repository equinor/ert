...
class JobManagerTestRuntimeKW(TestCase):
    ...
    def test_run_one_job_with_an_integer_arg_is_actually_a_fractional(self):
        ...
        self.assertFalse(starts[0].success(), "job should not start with success")
    ...
    def test_run_given_one_job_with_missing_file_and_one_file_present(self):
        ...
        self.assertTrue(starts[0].success(), "first job should start with success")
        self.assertFalse(
            starts[1].success(), "second job should not start with success"
        )

        exits = [e for e in statuses if isinstance(e, Exited)]
        self.assertEqual(1, len(exits), "There should be 1 exit message")
        self.assertTrue(exits[0].success(), "first job should exit with success")

        self.assertEqual(Finish, type(statuses[-1]), "last message should be Finish")
        self.assertFalse(statuses[-1].success(), "Finish status should not be success")
