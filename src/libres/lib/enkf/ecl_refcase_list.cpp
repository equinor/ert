...
ecl_refcase_list_type *ecl_refcase_list_alloc() {
    ecl_refcase_list_type *refcase_list =
        (ecl_refcase_list_type *)util_malloc(sizeof *refcase_list);
...
bool ecl_refcase_list_set_default(ecl_refcase_list_type *refcase_list,
    ...
    if (default_case) {
        ...
        if (default_pair) {
            ...
            hash_insert_hash_owned_ref(refcase_list->case_dict,
                                       default_pair->case_name, default_pair,
                                       sum_pair_free__);
            refcase_list->sorted = false;
            return true;
        } else
...
static void ecl_refcase_list_assert_clean(ecl_refcase_list_type *refcase_list) {
    if (!refcase_list->sorted) {
        ...
        {
            stringlist_type *tmp_list =
                hash_alloc_stringlist(refcase_list->case_dict);
            ...
            for (i = 0; i < stringlist_get_size(tmp_list); i++) {
                ...
                if (default_case &&
                    util_string_equal(casename, default_case->case_name))
                    normal_case = false;

                if (normal_case) {
                    sum_pair_type *pair = (sum_pair_type *)hash_get(
                        refcase_list->case_dict, casename);
...
static sum_pair_type *
ecl_refcase_list_get_pair(ecl_refcase_list_type *refcase_list,
                          const char *case_name) {
    ecl_refcase_list_assert_clean(refcase_list);
    {
        if (hash_has_key(refcase_list->case_dict, case_name))
            return (sum_pair_type *)hash_get(refcase_list->case_dict,
                                             case_name);
        else
            return NULL;
    }
...
static sum_pair_type *
ecl_refcase_list_iget_pair(ecl_refcase_list_type *refcase_list, int index) {
    ecl_refcase_list_assert_clean(refcase_list);
    {
        ...
        if (index < 0)
            ...
            return (sum_pair_type *)vector_safe_iget(refcase_list->case_list,
                                                     index);
    }
}
...
bool ecl_refcase_list_has_case(ecl_refcase_list_type *refcase_list,
                               const char *case_name) {
    const sum_pair_type *pair =
        ecl_refcase_list_get_pair(refcase_list, case_name);
    if (pair)
        return true;
    else
        return false;
...
const char *ecl_refcase_list_iget_pathcase(ecl_refcase_list_type *refcase_list,
    ...
    const ecl_sum_type *ecl_sum =
        ecl_refcase_list_iget_case(refcase_list, index);
    if (ecl_sum)
...
int ecl_refcase_list_add_case(ecl_refcase_list_type *refcase_list,
    ...
    if (pair) {
        if (hash_has_key(refcase_list->case_dict, pair->case_name)) {
            ...
            hash_insert_hash_owned_ref(refcase_list->case_dict, pair->case_name,
                                       pair, sum_pair_free__);
            refcase_list->sorted = false;
            return 1;
        }
    } else
...
int ecl_refcase_list_add_matching(ecl_refcase_list_type *refcase_list,
...

    {
        ...
        for (i = 0; i < stringlist_get_size(case_list); i++) {
            count += ecl_refcase_list_add_case(refcase_list,
                                               stringlist_iget(case_list, i));
        }
