...
int main(int argc, char **argv) {
    ...
    test_assert_int_equal(history_get_last_restart(hist_sim),
                          ecl_sum_get_last_report_step(refcase));
    test_assert_int_equal(history_get_last_restart(hist_h),
                          ecl_sum_get_last_report_step(refcase));

    {
        ...
        test_assert_true(
            history_init_ts(hist_sim, "FOPT", value_sim, valid_sim));
        test_assert_true(history_init_ts(hist_h, "FOPT", value_h, valid_h));
        {
            int step;
            for (step = 1; step < ecl_sum_get_last_report_step(refcase);
                 step++) {
                ...
                test_assert_true(bool_vector_iget(valid_h, step));
                {
                    ...
                    test_assert_double_equal(
                        ecl_sum_get_general_var(refcase, time_index, "FOPT"),
                        double_vector_iget(value_sim, step));
                    test_assert_double_equal(
                        ecl_sum_get_general_var(refcase, time_index, "FOPTH"),
                        double_vector_iget(value_h, step));
                }
            }
        }
