...
void test_create_invalid(const char *config_file) {
    ert_test_context_type *test_context =
        ert_test_context_alloc("CREATE_CONTEXT", config_file);
...
void test_create_valid(const char *config_file) {
    ...
    test_assert_true(
        enkf_main_is_instance(ert_test_context_get_main(test_context)));
    {
...
void test_install_job(const char *config_file, const char *job_file_OK,
                      const char *job_file_ERROR) {
    ert_test_context_type *test_context =
        ert_test_context_alloc("CREATE_CONTEXT_JOB", config_file);

    test_assert_false(ert_test_context_install_workflow_job(
        test_context, "JOB", "File/does/not/exist"));
    test_assert_false(ert_test_context_install_workflow_job(
        test_context, "ERROR", job_file_ERROR));
    test_assert_true(
        ert_test_context_install_workflow_job(test_context, "OK", job_file_OK));
...
void test_run_workflow_job(const char *config_file, const char *job_file) {
    ...
    test_assert_false(ert_test_context_run_worklow_job(
        test_context, "NO-this-does-not-exist", args1));
    ert_test_context_install_workflow_job(test_context, "JOB", job_file);

    test_assert_false(
        ert_test_context_run_worklow_job(test_context, "JOB", args0));
    test_assert_true(
        ert_test_context_run_worklow_job(test_context, "JOB", args1));
...
void test_install_workflow(const char *config_file, const char *job_file) {
    ert_test_context_type *test_context =
        ert_test_context_alloc("INSTALL_WORKFLOW", config_file);
    ...
    test_assert_true(
        ert_test_context_install_workflow(test_context, "WFLOW", wf_file));
    ert_test_context_free(test_context);
}
...
void test_run_workflow(const char *config_file, const char *job_file) {
    ert_test_context_type *test_context =
        ert_test_context_alloc("INSTALL_WORKFLOW", config_file);
    test_assert_false(
        ert_test_context_run_worklow(test_context, "No-does.not.exist"));
