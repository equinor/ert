...
void test_empty() {
    ...
    queue_config_type *queue_config_copy =
        queue_config_alloc_local_copy(queue_config);
...
void test_job_script() {
    ...
    queue_config_type *queue_config = queue_config_alloc_load(NULL);
    test_assert_false(
        queue_config_set_job_script(queue_config, "/does/not/exist"));
    ...
    test_assert_false(
        queue_config_set_job_script(queue_config, "DoesNotExits"));

    char *full_path = util_alloc_realpath("Script.sh");
    test_assert_string_equal(full_path,
...
void test_parse() {
    ...
    config_content_type *config_content =
        config_parse(parser, user_config_file, NULL, NULL, NULL, NULL,
    ...
    queue_driver_type *lsf_driver =
        queue_config_get_queue_driver(queue_config, LSF_DRIVER_NAME);
    test_assert_true(queue_driver_is_instance(lsf_driver));
    test_assert_string_equal(
        (const char *)queue_driver_get_option(lsf_driver, LSF_BJOBS_CMD),
        "the_path");

    test_assert_true(queue_config_has_job_script(queue_config));
    test_assert_string_equal(queue_config_get_queue_system(queue_config),
                             LSF_DRIVER_NAME);
    ...
    test_assert_true(queue_config_get_driver_type(queue_config_copy) ==
                     LOCAL_DRIVER);
