...
enkf_config_node_type *parse_alloc_GEN_DATA_result(const char *config_string,
    ...
    }
    {
        ...
        if (parse_valid) {
            const config_content_item_type *config_item =
                config_content_get_item(content, GEN_DATA_KEY);
            const config_content_node_type *config_node =
                config_content_item_iget_node(config_item, 0);

            enkf_config_node =
                enkf_config_node_alloc_GEN_DATA_from_config(config_node);
        }
...
void test_parse_gen_data_result() {
    ...
    {
        enkf_config_node_type *config_node =
            parse_alloc_GEN_DATA_result("GEN_DATA\n", false);
        test_assert_NULL(config_node);
    }

    // Validation error: missing INPUT_FORMAT:
    {
        enkf_config_node_type *config_node = parse_alloc_GEN_DATA_result(
            "GEN_DATA GEN_DATA_KEY  RESULT_FILE:Results%d  REPORT_STEPS:10 \n",
            true);
        test_assert_NULL(config_node);
    }
    ...
    {
        enkf_config_node_type *config_node = parse_alloc_GEN_DATA_result(
            "GEN_DATA GEN_DATA_KEY  RESULT_FILE:Results%d  "
            "INPUT_FORMAT:AsCiiiiii REPORT_STEPS:10 \n",
            true);
        test_assert_NULL(config_node);
    }
    ...
    {
        enkf_config_node_type *config_node = parse_alloc_GEN_DATA_result(
            "GEN_DATA GEN_DATA_KEY  INPUT_FORMAT:ASCII   REPORT_STEPS:10 \n",
            true);
        test_assert_NULL(config_node);
    }
    ...
    {
        enkf_config_node_type *config_node = parse_alloc_GEN_DATA_result(
            "GEN_DATA GEN_DATA_KEY  RESULT_FILE:Results INPUT_FORMAT:ASCII   "
            "REPORT_STEPS:10 \n",
            true);
        test_assert_NULL(config_node);
    }
    ...
    // Valid
    {
        ...
        test_assert_int_equal(DYNAMIC_RESULT,
                              enkf_config_node_get_var_type(config_node));
        {
            gen_data_config_type *gen_data_config =
                (gen_data_config_type *)enkf_config_node_get_ref(config_node);
            ...
            test_assert_int_equal(
                10, gen_data_config_iget_report_step(gen_data_config, 0));
            test_assert_int_equal(
                30, gen_data_config_iget_report_step(gen_data_config, 2));

            test_assert_true(
                gen_data_config_has_report_step(gen_data_config, 10));
            test_assert_true(
                gen_data_config_has_report_step(gen_data_config, 20));
            test_assert_true(
                gen_data_config_has_report_step(gen_data_config, 30));

            test_assert_false(
                gen_data_config_has_report_step(gen_data_config, 05));
            test_assert_false(
                gen_data_config_has_report_step(gen_data_config, 15));
            test_assert_false(
                gen_data_config_has_report_step(gen_data_config, 25));
            test_assert_false(
                gen_data_config_has_report_step(gen_data_config, 35));
        }
