...
struct enkf_fs_struct {
    ...
    bool read_only;
    std::shared_ptr<SparseTimeArray> datetime_index;
...
enkf_fs_type *enkf_fs_alloc_empty(const char *mount_point,
    ...
    fs->datetime_index = std::make_shared<SparseTimeMap>();
...
static void enkf_fs_fsync_time_map(enkf_fs_type *fs) {
    ...
    time_array::write(filename, *fs->datetime_index);
    free(filename);
}
...
std::shared_ptr<SparseTimeArray> enkf_fs_get_time_map(const enkf_fs_type *fs) {
    return fs->datetime_index;
}
