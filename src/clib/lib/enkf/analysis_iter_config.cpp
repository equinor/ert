...
void analysis_iter_config_set_num_retries_per_iteration(
    analysis_iter_config_type *config, int num_iter_tries) {
...
bool analysis_iter_config_case_fmt_set(
    const analysis_iter_config_type *config) {
...
analysis_iter_config_type *analysis_iter_config_alloc() {
    analysis_iter_config_type *config =
        (analysis_iter_config_type *)util_malloc(sizeof *config);
    ...
    analysis_iter_config_set_num_iterations(config,
                                            DEFAULT_ANALYSIS_NUM_ITERATIONS);
    analysis_iter_config_set_num_retries_per_iteration(
        config, DEFAULT_ITER_RETRY_COUNT);
...
analysis_iter_config_type *analysis_iter_config_alloc_full(const char *case_fmt,
                                                           int num_iterations,
                                                           int num_iter_tries) {
    analysis_iter_config_type *config =
        (analysis_iter_config_type *)util_malloc(sizeof *config);
...
void analysis_iter_config_init(analysis_iter_config_type *iter_config,
    ...
    if (config_content_has_item(config, ITER_CASE_KEY))
        analysis_iter_config_set_case_fmt(
            iter_config, config_content_get_value(config, ITER_CASE_KEY));

    if (config_content_has_item(config, ITER_COUNT_KEY))
        analysis_iter_config_set_num_iterations(
            iter_config,
            config_content_get_value_as_int(config, ITER_COUNT_KEY));

    if (config_content_has_item(config, ITER_RETRY_COUNT_KEY))
        analysis_iter_config_set_num_retries_per_iteration(
            iter_config,
            config_content_get_value_as_int(config, ITER_RETRY_COUNT_KEY));
}
