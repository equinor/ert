    ...
    def __init__(self, workflow_list, config_content=None, config_dict=None):
        ...
        if config_dict is not None:
            ...
            if ConfigKeys.HOOK_WORKFLOW_KEY in config_dict:
                for hook_workflow_name, run_mode_name in config_dict[ConfigKeys.HOOK_WORKFLOW_KEY]:
                    if run_mode_name not in [runtime.name for runtime in HookRuntime.enums()]:
                        raise ValueError("Run mode {} not supported".format(run_mode_name))
