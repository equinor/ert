...
class JobRunner(object):
    ...
    def __init__(self, jobs_file="jobs.json", sleep_interval=5):
        ...
        os.umask(int(self.jobs_data["umask"], 8))

        self._data_root = self.jobs_data.get("DATA_ROOT")
        if self._data_root:
            os.environ["DATA_ROOT"] = self._data_root

        self.simulation_id = self.jobs_data.get("run_id")
        self.ert_pid = self.jobs_data.get("ert_pid")
        self.global_environment = self.jobs_data.get("global_environment")
        self.global_update_path = self.jobs_data.get("global_update_path")
        self.job_data_list = self.jobs_data["jobList"]

        if self.simulation_id is not None:
            os.environ["ERT_RUN_ID"] = self.simulation_id

        self.jobs = []
        for index, job_data in enumerate(self.job_data_list):
    ...
    def run(self, names_of_jobs_to_run):
        ...
        yield Finish() if all_jobs_successful else Finish().with_error(
