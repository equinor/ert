...
class ErtTestContextTest(ResTest):
    ...
    def loadResultsTest(self, context):
        load_results_job = self.createSharePath(
            "ert/workflows/jobs/internal/config/LOAD_RESULTS"
        )
    ...
    def rankRealizationsOnObservationsTest(self, context):
        rank_job = self.createSharePath(
            "ert/workflows/jobs/internal/config/OBSERVATION_RANKING"
        )
