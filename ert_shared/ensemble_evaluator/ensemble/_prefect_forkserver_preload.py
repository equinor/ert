...
macOS SDK before the forkserver ever calls fork(). This is only viable when code executed
post-fork is known. This is likely to break down if the ert3 user introduces
arbitrary code execution in a FunctionStep. See https://github.com/equinor/ert/issues/2045
"""
...
if sys.platform == "darwin":
    ...
    # Ensure requests make its _scproxy [1] calls so that they don't have to
    # be repeated after the fork(). StorageTransmitters will at the very least
    # make this call, so it needs to be made here as well.
