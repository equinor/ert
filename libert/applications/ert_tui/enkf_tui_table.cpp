...
static void enkf_tui_table__(enkf_main_type * enkf_main , bool gen_kw_table , bool ens_plot) {
  ...
  active       = (bool *) util_calloc( num_keys , sizeof * active       );
  nodes        = (enkf_node_type **) util_calloc( num_keys , sizeof * nodes        );
  config_nodes = (enkf_config_node_type **) util_calloc( num_keys , sizeof * config_nodes );
  index_keys   = (char **) util_calloc( num_keys , sizeof * index_keys   );
  ...
  index = (int *) util_calloc( length   , sizeof * index );
  data  = (double **) util_calloc( num_keys , sizeof * data  );
  {
    int i;
    for (i = 0; i < num_keys; i++)
      data[i] = (double *) util_calloc( length , sizeof * data[i] );
  }
